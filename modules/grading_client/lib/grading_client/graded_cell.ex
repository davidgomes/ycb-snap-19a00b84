  ...
  def to_source(attrs) do
    ...
    source_ast =
      try do
        ...
        module_id =
          case modules[String.trim(module_id_str)] do
            nil -> raise "invalid module id: #{module_id_str}"
            module_id -> module_id
          end

        question_id =
          case Integer.parse(String.trim(question_id_str)) do
            {id, ""} -> id
            _ -> raise "invalid question id: #{question_id_str}"
          end

        quote do
          result = unquote(source)

          case GradingClient.check_answer(
                 unquote(Macro.escape(module_id)),
                 unquote(question_id),
                 result
               ) do
            :correct ->
