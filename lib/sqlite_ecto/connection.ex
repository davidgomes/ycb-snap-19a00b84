if Code.ensure_loaded?(Sqlitex.Server) do
  defmodule Sqlite.Ecto2.Connection do
    ...
    defp returning(%Query{select: %{fields: fields}} = query, sources, cmd) do
      ...
      fields = Enum.map_join([table | returning_fields(fields, query)], ",", &quote_id/1)
      [@pseudo_returning_statement, cmd, ?\s, fields]
    end

    defp returning_fields(fields, query) do
      Enum.map(fields, fn
        {{:., _, [{:&, _, [0]}, field]}, _, []} ->
          field
        _ ->
          error!(query, "SQLite adapter can only return fields from the table being modified")
      end)
    end
    ...
    defp column_options(_default, type, _, true)
      when type in [:serial, :bigserial]
    do
