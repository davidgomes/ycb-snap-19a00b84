defmodule PetalComponents.DataTable.State do
  ...
  def handle_op(%__MODULE__{} = state, params, opts) when is_map(params) and is_list(opts) do
    ...
    case params do
      ...
      %{"op" => "select_page", "ids" => ids} when is_list(ids) ->
        toggle_page_selected(state, ids)
  ...
  @doc """
  Toggles one row id in the selection. Ids are kept as strings (what the
  DOM round-trips), so integer and string ids compare equal. Selection
  is session state: it survives paging, sorting and filtering, and never
  round-trips through `to_params/1`.
  """
  ...
  @doc """
  The tri-state header's click: when every id in `ids` (the visible
  page) is selected they are all deselected, otherwise the missing ones
  are added. Selections on other pages are untouched.
  """
  def toggle_page_selected(%__MODULE__{} = state, ids) do
  ...
  @doc """
  The header checkbox's state for the visible `ids`: `:all`, `:some`
  (indeterminate) or `:none`.
  """
  def selection_state(%__MODULE__{selected: selected}, ids) do
    ids = Enum.map(ids, &to_string/1)
    count = Enum.count(ids, &(&1 in selected))

    cond do
      ids != [] and count == length(ids) -> :all
      count > 0 -> :some
      true -> :none
    end
  end
