defmodule PetalComponents.DataTable do
  ...
  def data_table(assigns) do
    ...
    # link mode: URL wiring for search / page-size / filters. Either
    # mode: selection's tri-state header checkbox needs its
    # indeterminate property set. Filter menus are in-page popovers -
    # event mode closes them with JS.hide on submit, no hook required.
    hooked? =
      (link_mode? and
  ...
  # event mode pushes the form and hides the in-page panel; link mode
  # has no events, so Apply is the hook's job (patch + display:none)
