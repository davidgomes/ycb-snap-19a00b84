defmodule PetalComponents.DataTable do
  ...
  `select_all`, `clear_selection` - a MapSet plus a handful of clauses
  is the whole backend. Selection is keyed by `row_id`, which must
  uniquely identify a record across the whole dataset, not just the
  current page (primary keys qualify, display fields do not) - a
  selection retained while paging is only as sound as this key.
  Same-page duplicates raise; cross-page uniqueness is the caller's
  contract, exactly as with LiveView stream ids.
  ...
  def data_table(assigns) do
    ...
    # checkbox's indeterminate property (which markup cannot express)
    # is mirrored from a data attr.
    hooked? =
  ...
  defp selection_assigns(assigns) do
    ...
    selected_ids =
      assigns.selected |> Enum.map(&stringify_id/1) |> Enum.reject(&(&1 == "")) |> Enum.uniq()
  ...
  defp row_key(row, fun) when is_function(fun, 1), do: stringify_id(fun.(row))
  defp row_key(row, field) when is_atom(field), do: stringify_id(Map.get(row, field))
