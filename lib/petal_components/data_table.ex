defmodule PetalComponents.DataTable do
  ...
  mirrors the header checkbox's indeterminate state in both modes,
  because that is a DOM property markup cannot express.
  ...
  `select_all`, and `clear_selection`. `State.handle_op/3` ignores
  them on purpose, so the handler is a list of string ids plus three
  clauses. Selection is keyed by `row_id`, which must uniquely
  ...
  def data_table(assigns) do
    ...
    # checkbox's indeterminate property (markup cannot express it) is
    # mirrored from data-pc-dt-indeterminate.
    hooked? =
