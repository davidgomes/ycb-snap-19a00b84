defmodule PetalComponents.DataTable do
  ...
  `select_all` and `clear_selection`. A MapSet plus three clauses is
  the whole backend. Selection is keyed by `row_id`, which must uniquely
  ...
  def data_table(assigns) do
    ...
    # top-layer popovers the hook closes after an Apply, and selectable
    # tables mount the hook so it can mirror the header checkbox's
    # indeterminate property, which markup alone cannot express.
    hooked? =
