defmodule PetalComponents.DataTable do
  ...
  def data_table(assigns) do
    ...
    # link mode: URL wiring. Either mode: selection's tri-state header
    # checkbox needs its indeterminate property set.
    hooked? =
      (link_mode? and
  ...
  # Apply closes the editor in both modes. It must be a JS command, not a
  # style write: LiveView re-applies the last JS show/hide on every patch,
  # so a hand-hidden panel would reopen on the patch the Apply causes.
  # Event mode also pushes the form; link mode's hook turns the same
  # submit into a patch.
  defp filter_submit_js(event, target, pop_id) do
    trigger = "##{pop_id}-trigger"

    close =
      JS.hide(to: "##{pop_id}")
      |> JS.set_attribute({"aria-expanded", "false"}, to: trigger)
      |> JS.focus(to: trigger)

    cond do
      is_nil(event) -> close
      target -> JS.push(close, event, target: target)
      true -> JS.push(close, event)
    end
  end
