defmodule Sentry.Transport.RateLimiter do
  ...
  @doc """
  Checks if items of the given data category are currently rate-limited.

  Like `rate_limited?/1`, but also takes into account the byte-based category
  associated with the given data category (such as `log_byte` for `log_item`),
  since a limit on either one means the items must not be sent.
  ...
  @doc """
  Returns the byte-based data category associated with the given data category,
  or `nil` if there isn't one.

  ## Examples

      iex> RateLimiter.byte_category("log_item")
      "log_byte"

      iex> RateLimiter.byte_category("error")
      nil

  """
  @spec byte_category(String.t()) :: String.t() | nil
  def byte_category("log_item"), do: "log_byte"
  def byte_category("trace_metric"), do: "trace_metric_byte"
  def byte_category(_category), do: nil
