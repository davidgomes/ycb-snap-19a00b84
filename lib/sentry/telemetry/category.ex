defmodule Sentry.Telemetry.Category do
  ...
  serialized size of dropped items in client reports and to honor byte-based rate limits.
  Only `:log` and `:metric` currently have such companion categories.

  These strings are used in client reports and rate limiting alongside the count-based
  category returned by `data_category/1`.
