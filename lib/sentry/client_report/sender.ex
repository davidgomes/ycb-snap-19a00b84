defmodule Sentry.ClientReport.Sender do
  ...
  def record_discarded_events(reason, event_items, genserver)
    ...
    if Enum.member?(@client_report_reasons, reason) do
      # Byte outcomes require JSON-encoding each item, and callers include hot
      # paths such as every `Logger` call under an active rate limit, so the
      # items are expanded into outcomes by this GenServer rather than the caller.
    ...
    discarded_events =
      Enum.reduce(items, discarded_events, fn item, acc ->
        Enum.reduce(data_categories(item), acc, fn
          # Client reports require a positive quantity, so outcomes we can't
          # quantify (an empty batch, or items that failed to encode) are skipped.
          {_category, 0}, acc -> acc
