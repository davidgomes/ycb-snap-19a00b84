defmodule Hammer.ETS do
  ...
  - `:before_clean` - optional callback invoked with the algorithm name and the expired entries right before they are removed, see `start_link/1`
  ...
  Accepts the following options:
    ...
    - `:before_clean` - Optional callback invoked during cleanup with the algorithm name
      (`:fix_window`, `:sliding_window`, `:leaky_bucket` or `:token_bucket`) and the list of
      expired entries, right before they are removed. Accepts a 2-arity function or a
      `{module, function, extra_args}` tuple (called as `apply(module, function, [algorithm, entries | extra_args])`).
      It runs inside the cleaning process and is not called when nothing expired.
      If it raises, a warning is logged and the entries are removed anyway.
      Entries are maps whose shape depends on the algorithm:
        - `:fix_window` - `%{key: key, window: window, count: count, expires_at: ms}`
        - `:sliding_window` - `%{key: key, timestamp: us, expires_at: us}` (one entry per hit, in microseconds)
        - `:leaky_bucket` - `%{key: key, level: level, last_update: seconds}`
        - `:token_bucket` - `%{key: key, tokens: tokens, last_update: seconds}`
    - optional `:debug`, `:spawn_opts`, and `:hibernate_after` GenServer options

  Example:

      MyApp.RateLimit.start_link(
        before_clean: fn :fix_window, entries ->
          Enum.each(entries, &MyApp.Billing.record_usage(&1.key, &1.count))
        end
      )
  """
  ...
  defp clean(%{algorithm: algorithm, table: table} = config) do
    expired = algorithm.select_expired(config)

    CleanUtils.invoke_before_clean(
      config.before_clean,
      CleanUtils.algorithm_name(algorithm),
      algorithm.normalize_expired(expired)
    )

    # delete_object/2 leaves entries that were updated since they were selected
