defmodule Hammer.Atomic do
  ...
  defp clean_fix_window(config) do
    ...
    maybe_invoke_before_clean(config.before_clean, config.algorithm_module, expired_terms)
    Hammer.CleanUtils.delete_expired(config.table, expired_terms)
  end
  ...
  defp maybe_invoke_before_clean(nil, _algorithm_module, _expired_terms), do: :ok
  defp maybe_invoke_before_clean(_callback, _algorithm_module, []), do: :ok

  defp maybe_invoke_before_clean(callback, algorithm_module, expired_terms) do
    entries =
      Enum.map(expired_terms, fn {key, atomic} ->
        algorithm_module.normalize_entry(key, atomic)
      end)
