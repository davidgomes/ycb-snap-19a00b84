defmodule Hammer.CleanUtils do
  ...
  @doc false
  @spec invoke_before_clean(before_clean(), atom(), [map()]) :: any()
  ...
  @doc false
  @spec delete_expired(atom(), [tuple()]) :: non_neg_integer()
  def delete_expired(table, expired) do
    Enum.each(expired, fn object -> :ets.delete_object(table, object) end)
    length(expired)
  end
end
