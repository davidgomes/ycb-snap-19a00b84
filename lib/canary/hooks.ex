if Code.ensure_loaded?(Phoenix.LiveView) do
  defmodule Canary.Hooks do
    ...
    For the authorization actions, when the `:required` is false (by default it's true) the resource might be nil.
    ...
    * `:unauthorized_handler` - Specify a handler function to be called if the action is unauthorized
    ...
    defp fetch_resource(%Socket{} = socket, opts) do
      resource_name = get_resource_name(socket.assigns.canary_action, opts)

      case Map.get(socket.assigns, resource_name, nil) do
