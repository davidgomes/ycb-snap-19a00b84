defmodule Canary.Plugs do
  ...
  be overridden to fetch a single record from the database by setting `required: true`.
  ...
  by setting `required: true`.

  Setting `required: true` explicitly overrides how a resource is loaded and can be useful when dealing
  ...
  * `:persisted` - Deprecated, use `:required` instead
  * `:not_found_handler` - Specify a handler function to be called if the resource is not found
  ...
  plug :load_resource, model: Post, id_name: "slug", id_field: "slug", only: [:show]
  ```
  """
  ...
  * `:unauthorized_handler` - Specify a handler function to be called if the action is unauthorized
  ...
  the module name only if `required: true` is set and you want to override the default
  ...
  For example:
    ...
    To authorize other actions based on the model name, use a separate plug with `required: false`
    (the deprecated `:non_id_actions` opt is still supported).
  ...
  * `:id_name` - Specifies the name of the id in `conn.params`, defaults to "id"
  * `:id_field` - Specifies the name of the ID field in the database for searching :id_name value, defaults to "id".
  * `:required` - Specifies the resource should always be loaded from the database when set to true
  * `:persisted` - Deprecated, use `:required` instead
  * `:non_id_actions` - Deprecated, use a separate plug with `required: false` instead
  * `:unauthorized_handler` - Specify a handler function to be called if the action is unauthorized
  ...
  plug :authorize_resource, model: Post, id_name: "slug", id_field: "slug", only: [:show]
  ```
  """
  ...
  * `:not_found_handler` - Specify a handler function to be called if the resource is not found
  ...
  plug :load_and_authorize_resource, model: Post, id_name: "slug", id_field: "slug", only: [:show]
  ```
  """
  ...
  # The not found handler runs after authorization so the unauthorized handler can catch first
  ...
  defp handle_not_found(conn, opts) do
    if apply_handle_not_found?(get_action(conn), conn.assigns, opts) do
