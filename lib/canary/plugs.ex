defmodule Canary.Plugs do
  ...
  * `:persisted` - Deprecated, specifies the resource should always be loaded from the database, defaults to false
  * `:required` - Specifies if the resource is required, when it's not found it will call the not found handler - even for :index, :new or :create action, defaults to true
  ...
  * `:not_found_handler` - Specify a handler function to be called if the resource is not found
  ...
  defp do_load_and_authorize_resource(conn, opts) do
    conn
    # not found is handled after authorization so the unauthorized handler can catch first
    |> do_load_resource(opts)
