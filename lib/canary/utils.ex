defmodule Canary.Utils do
  ...
  it when needed — only for the `:index` action.

  If the `:as` option is provided, it is used as the resource name.
  ...
  @doc """
  Returns the non-id actions, including any extra actions from `:non_id_actions`.
  """
  ...
  defp warn_deprecated_opts(opts) do
    ...
    if Keyword.has_key?(opts, :non_id_actions) do
      IO.warn(
        "The `:non_id_actions` option is deprecated and will be removed in Canary 2.1.0. Use a separate :authorize_resource plug for non-id actions and `:except` to skip them on load. Check the documentation for more information."
      )
    end
