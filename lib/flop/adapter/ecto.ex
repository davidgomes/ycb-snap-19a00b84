defmodule Flop.Adapter.Ecto do
  ...
  def apply_filter(
    ...
    case get_field_info(schema_struct, field) do
      ...
      %FieldInfo{
        ...
        field = apply(mod, fun, [dynamic_opts])

        Query.where(
          query,
          ^build_op(
            schema_struct,
            %{field_info | extra: Map.put(field_info.extra, :field, field)},
            filter,
            dialect(opts)
          )
        )
  ...
  for op <- @operators, op not in [:empty, :not_empty | @ilike_operators] do
    ...
    dynamic_fragment = Flop.Adapter.Ecto.Operators.field_as_dynamic(fragment)

    defp build_op(
           _schema_struct,
           %FieldInfo{extra: %{type: :custom, field: field}},
           %Filter{op: unquote(op), value: value},
           _dialect
         ) do
      unquote(prelude)
