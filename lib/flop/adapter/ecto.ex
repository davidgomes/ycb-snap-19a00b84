defmodule Flop.Adapter.Ecto do
  ...
  def apply_filter(
    ...
    case get_field_info(schema_struct, field) do
      ...
      %FieldInfo{
        ecto_type: ecto_type,
        extra: %{type: :custom, field_dynamic: {_, _, _} = field_dynamic}
      } ->
        Query.where(
          query,
          ^build_dynamic_op(
            custom_field_dynamic(field_dynamic, opts),
      ...
      %FieldInfo{extra: %{type: :custom}} ->
        ...
        Neither function is configured for #{inspect(field)}, so it cannot be
  ...
  ## Filter query builder for field_dynamic expressions of custom fields
