defmodule Flop.Adapter.Ecto do
  ...
  defp build_op(
         ...
         %FieldInfo{
           ecto_type: ecto_type,
           extra: %{type: :custom, dynamic: field_dynamic}
         },
         %Filter{op: op, value: value},
         dialect
       )
    ...
    condition =
      case {array_or_map(ecto_type), dialect} do
        {:array, %Dialect{arrays?: false}} ->
          dynamic(
            is_nil(^field_dynamic) or
              fragment("JSON_LENGTH(?) = 0", ^field_dynamic)
          )

        {:array, _} ->
          empty_value = dynamic(type(^[], ^ecto_type))
          dynamic(is_nil(^field_dynamic) or ^field_dynamic == ^empty_value)
  ...
  defp build_op(
       ...
       when op in [:contains, :not_contains] do
    dumped_value =
      Dialect.dump_array_element(value, Flop.Misc.expand_type(ecto_type))

    condition =
      dynamic(fragment("JSON_CONTAINS(?, ?)", ^field_dynamic, ^[dumped_value]))
  ...
  for op <- @operators, op not in [:empty, :not_empty | @ilike_operators] do
    ...
    defp build_op(
      ...
      build_dynamic(
        unquote(with_field_dynamic(fragment)),
        false,
        unquote(combinator)
      )
    end
  ...
  defp validate_custom_field_callback!(custom_fields, fields, callbacks, usage) do
    ...
    names = Enum.join(callbacks, " or ")

    if missing != [] do
      raise ArgumentError, """
      custom field without #{names} function marked as #{usage}

      A custom field needs a #{names} function to be #{usage}. These fields
      have none:
