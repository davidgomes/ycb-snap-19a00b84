defmodule Flop.Adapter.Ecto do
  ...
  def apply_filter(
    ...
    case get_field_info(schema_struct, field) do
      %FieldInfo{
        ...
        apply(mod, fun, [query, filter, custom_opts(opts, custom_filter_opts)])

      # only reachable with an unvalidated Flop struct
      %FieldInfo{extra: %{type: :custom, filter: nil}} ->
        raise ArgumentError, """
        custom field #{inspect(field)} has no filter function

        Set the :filter option of the custom field to make it filterable.
  ...
  defp custom_opts(opts, field_opts) do
    opts
  ...
  defp apply_order_by_field(
    ...
    field_dynamic = apply(mod, fun, [custom_opts(opts, field_opts)])
    order_by_direction(q, order_direction, field_dynamic)
  end
  ...
  defp apply_order_by_field(
    ...
    raise ArgumentError, """
    custom field #{inspect(field)} has no field_dynamic function

    Set the :field_dynamic option of the custom field to make it sortable.
  ...
  defp validate_custom_fields!(
    ...
    unfilterable_fields =
      fields_without_option(custom_fields, :filter, filterable)

    if unfilterable_fields != [] do
      raise ArgumentError, """
      cannot filter by custom fields without filter function

      Custom fields need the :filter option to be filterable. These custom fields
      were configured as filterable without a filter function:
    ...
    unsortable_fields =
      fields_without_option(custom_fields, :field_dynamic, sortable)

    if unsortable_fields != [] do
      raise ArgumentError, """
      cannot sort by custom fields without field_dynamic function

      Custom fields need the :field_dynamic option to be sortable. These custom
      fields were configured as sortable without a field_dynamic function:
  ...
  defp fields_without_option(custom_fields, option, fields) do
    for {name, %{^option => nil}} <- custom_fields,
        name in fields,
        do: name
  end
