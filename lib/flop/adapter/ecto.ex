defmodule Flop.Adapter.Ecto do
  ...
  def apply_filter(
    ...
    case get_field_info(schema_struct, field) do
      ...
      %FieldInfo{
        ...
        field_expr = apply(mod, fun, [dynamic_opts])
        ...
        Query.where(
          ...
          ^build_op(
            ...
            %FieldInfo{
              ecto_type: ecto_type,
              extra: %{type: :dynamic, field_expr: field_expr}
            },
            filter,
            dialect(opts)
          )
  ...
  for op <- @operators, op not in [:empty, :not_empty | @ilike_operators] do
    {fragment, prelude, combinator} = op_config(op, true, :expr)

    defp build_op(
           _schema_struct,
           %FieldInfo{extra: %{type: :dynamic, field_expr: field_expr}},
           %Filter{op: unquote(op), value: value},
           _dialect
         ) do
      unquote(prelude)
  ...
  for op <- @ilike_operators, ilike? <- [true, false] do
    {fragment, prelude, combinator} = op_config(op, ilike?, :expr)
