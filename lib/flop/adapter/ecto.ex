defmodule Flop.Adapter.Ecto do
  ...
  defp apply_order_by_field(
    ...
    case Map.get(custom_opts, :field_dynamic) do
      {mod, fun, field_dynamic_opts} ->
        opts =
          opts
          |> Keyword.get(:extra_opts, [])
      ...
      nil ->
        ...
        raise ArgumentError, """
        cannot order by custom field #{inspect(field)}

        Custom fields can only be used for ordering if the :field_dynamic option
        is set.
  ...
  defp validate_custom_fields!(
    ...
    illegal_fields =
      custom_fields
      |> Enum.filter(fn {field, %{field_dynamic: field_dynamic}} ->
        field in sortable and is_nil(field_dynamic)
      end)
    ...
    if illegal_fields != [] do
      ...
      Set :field_dynamic to a module/function/options tuple referencing a
      function that returns the field as an Ecto.Query.dynamic_expr.
      """
    end
