defmodule Flop.Adapter.Ecto do
        ...
        field_info = %{
          ...
          | extra:
              Map.put(
                field_info.extra,
                :field_expr,
                apply(mod, fun, [field_dynamic_opts])
              )
        }
  ...
  defp build_op(
    ...
    condition =
      case {array_or_map(ecto_type), dialect} do
        {:array, %Dialect{arrays?: false}} -> dynamic(empty_dynamic(:json_array))
        {:array, _} -> dynamic(empty_dynamic(:array))
        {:map, _} -> dynamic(empty_dynamic(:map))
        {:other, _} -> dynamic(empty_dynamic(:other))
      end
  ...
  for op <- @operators, op not in [:empty, :not_empty | @ilike_operators] do
    {fragment, prelude, combinator} = op_config_dynamic(op)
    ...
    defp build_op(
      ...
      unquote(prelude)
      build_dynamic(unquote(fragment), :expr, unquote(combinator))
    end
  end
  ...
  for op <- @ilike_operators, ilike? <- [true, false] do
    {fragment, prelude, combinator} = op_config_dynamic(op, ilike?)
  ...
  defp validate_custom_field_callback!(custom_fields, fields, callbacks, usage)
    ...
    if missing != [] do
      ...
      raise ArgumentError, """
      custom field without #{Enum.join(callbacks, " or ")} function marked as #{usage}

      A custom field needs a #{Enum.join(callbacks, " or ")} function to be
      #{usage}. These fields have none:
