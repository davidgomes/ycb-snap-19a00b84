defmodule Flop.Adapter.Ecto do
  ...
  defp field_source(field, _field_info), do: {:column, field}

  # NULL cursor value, nulls last, last cursor field. Every remaining row
  # shares this NULL, so there is no later position without a tiebreaker.
  defp seek(_direction, :last, nil, _source, nil), do: false

  # NULL cursor value, nulls last, more cursor fields to come
  defp seek(_direction, :last, nil, source, tail) do
    dynamic(^null_dynamic(source) and ^tail)
  end

  # NULL cursor value, nulls first, last cursor field
  defp seek(_direction, :first, nil, source, nil), do: not_null_dynamic(source)

  # NULL cursor value, nulls first, more cursor fields to come
  ...
  # ascending, nulls last: NULL rows follow the cursor row
  ...
  # descending, nulls last: NULL rows follow the cursor row
  defp strictly_after(_direction, :last, cursor_value, source) do
    dynamic(^compare(:<, source, cursor_value) or ^null_dynamic(source))
  end

  # ascending, nulls first: the cursor row has passed the NULL rows
  defp strictly_after(direction, :first, cursor_value, source)
       when direction in @asc_directions do
    compare(:>, source, cursor_value)
  end

  # descending, nulls first: the cursor row has passed the NULL rows
