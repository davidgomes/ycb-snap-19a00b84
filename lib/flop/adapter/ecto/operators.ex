defmodule Flop.Adapter.Ecto.Operators do
  ...
  def op_config(:starts_with, false) do
    ...
    fragment_dynamic = like_fragment_dynamic(quote(do: ^var!(value)))
  ...
  def op_config(:like_and) do
    ...
    fragment_dynamic = like_fragment_dynamic(quote(do: ^substring))
    combinator = :and
  ...
  defmacro empty_dynamic(:array) do
    quote do
      is_nil(^var!(field_dynamic)) or
        type(^var!(field_dynamic), ^var!(ecto_type)) ==
          type(^[], ^var!(ecto_type))
    end
  end
