defprotocol Flop.Schema do
  ...
  - `field_dynamic` returns the field as an `Ecto.Query.dynamic_expr`. It
    receives an options keyword list and neither the query nor the direction.
    Flop applies the order direction to the expression, and, when no `filter`
    callback is set, the built-in filter operators. A custom field needs it to
    be sortable.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    the order direction to the expression, and uses it with the built-in filter
    operators when `:filter` is not set. Required if the field is sortable.
