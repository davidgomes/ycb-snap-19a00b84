defprotocol Flop.Schema do
  ...
  functions. Options passed in the tuple override `extra_opts` when a key is
  set in both.
  ...
  `field_dynamic` returns:
      ...
      dynamic =
        CustomFilters.date_field(source: :inserted_at, timezone: timezone)
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    applies the ordering clauses and the filter operators on the expression.
    Required if the field is sortable. One of the two is required if it is
    filterable.
