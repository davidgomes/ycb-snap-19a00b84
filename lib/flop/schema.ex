defprotocol Flop.Schema do
  ...
  Custom fields support filtering and sorting. Sorting uses a `:sorter` tuple
  with the same shape as `:filter`. The referenced function receives the Ecto
  query, the order direction (`:asc`, `:desc`, `:asc_nulls_first`,
  `:asc_nulls_last`, `:desc_nulls_first` or `:desc_nulls_last`), and the
  options keyword list. It must return the query. Flop does not rewrite null
  ordering for a custom sorter; the function applies the direction itself.

  A field needs `:filter` when it is filterable and `:sorter` when it is
  sortable. Either option can be omitted when the field is not used that way.
  Cursor pagination does not support custom fields.
        ...
        adapter_opts: [
          custom_fields: [
            inserted_at_date: [
              ...
              sorter: {CustomFilters, :date_sorter, [source: :inserted_at]},
              ecto_type: :date,
              operators: [:<=, :>=]
            ]
          ]
  ...
  Filter module:
      ...
      defmodule CustomFilters do
        ...
        def date_sorter(query, direction, opts) do
          ...
          order_by(query, [r], [{^direction, expr}])
        end
      end
  ...
  - `:sortable` (required) - A list of fields that can be used for sorting.
    ...
    fields, and custom fields that define a `:sorter`.
  ...
  - `:sorter` - A module/function/options tuple referencing a custom sorter
    function. Required when the field is sortable. The function must take the
    Ecto query, the order direction, and the options from the tuple as
    arguments, and return the query.
