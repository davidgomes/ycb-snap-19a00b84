defprotocol Flop.Schema do
  ...
  conditions and order fields.

  ### Ordering by custom fields

  To make a custom field sortable, set the `:field_dynamic` option. It
  references a function that receives an options keyword list and returns the
  expression to order by as an `Ecto.Query.dynamic_expr`. As with the filter
  function, the options from the tuple are merged into the `extra_opts`.
  ...
  Custom fields cannot be used as order fields with cursor-based pagination.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    that returns the expression to order by as an `Ecto.Query.dynamic_expr`.
    The function takes the options from the tuple, merged into the
    `extra_opts`, as its only argument. Required if the field is sortable.
  - `:ecto_type` (required) - The Ecto type of the field. The filter operator
    and value validation is based on this option.
  - `:bindings` - If the custom filter or field dynamic requires certain named
    ...
    field is used in a filter or as an order field.
