defprotocol Flop.Schema do
  ...
  - `field_dynamic` is called to order by the field. It receives an options
    ...
    custom field needs it to be sortable. If no `filter` function is set, Flop
    also uses the expression to filter by the field with the regular filter
    operators.

  A custom field needs at least one of the two callbacks to be filterable.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    If no `:filter` function is set, Flop also applies filters to the
    expression, using the same operators as for regular fields.
