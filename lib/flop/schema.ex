defprotocol Flop.Schema do
  ...
  - `field_dynamic` is called to order or filter by the field. It receives an
    ...
    the order direction or the filter operator and value to the expression, the
    same way it does for a schema field. It receives neither the query nor the
    direction or filter. A custom field needs it to be sortable.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    the order direction, or the filter operator and value, to the expression.
    Required if the field is sortable.
