defprotocol Flop.Schema do
  ...
  it possible to implement logic that the built-in filtering and ordering
  options cannot satisfy.
  ...
  Custom field functions are referenced by a tuple
  `{mod :: module, function :: atom, opts :: keyword}`.

  - The `:filter` function receives three arguments: the Ecto query, the Flop
    filter, and an options keyword list. It must return the filtered query. This
    option is required to make a custom field filterable.
  - The `:field_dynamic` function receives the options keyword list as its only
    argument. It must return the field expression as an
    `Ecto.Query.dynamic_expr`, and Flop applies the order clauses on it. This
    option is required to make a custom field sortable.
  ...
  functions. The extra options are merged with the options from the tuple.

  Custom fields cannot be used as order fields with cursor pagination.
  ...
  If your custom field functions require certain named bindings, you can use the
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    function must take the options from the tuple as its only argument. Flop
    applies the order clauses on the expression. Required if the field is
    sortable.
  - `:ecto_type` (required) - The Ecto type of the field. The filter operator
    and value validation is based on this option.
  - `:bindings` - If the custom field functions require certain named bindings
    ...
    is used in a filter or as an order field.
