defprotocol Flop.Schema do
  ...
  Custom fields can also be used for ordering. Set `:field_dynamic` to a
  module/function/options tuple. The function receives that options keyword
  list and returns the field as an `Ecto.Query.dynamic_expr()`. Flop applies
  the order direction to the expression. `:field_dynamic` is required when the
  field is listed in `:sortable`. A custom field can define `:filter`,
  `:field_dynamic`, or both. `:filter` is used for filtering and
  `:field_dynamic` for ordering.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    that returns the field as an `Ecto.Query.dynamic_expr()`. The function
    takes the options keyword list as its only argument. Flop orders by the
    returned expression. Required if the field is sortable.
