defprotocol Flop.Schema do
  ...
  - `field_dynamic` is called to order or filter by the field. It receives an
    ...
    applies the order direction or the filter operator to. It receives neither
    the query nor the direction or filter. A custom field needs it to be
    sortable.
  ...
  The `date_filter` function in this example only compares the expression
  returned by `date_field` with the filter value, so it could be omitted. Without
  a `filter` function, Flop applies the filter operator to the `field_dynamic`
  expression, the same way it does for a schema field.
  ...
  - `:filter` - A module/function/options tuple referencing a custom filter
    ...
    Either this option or `:field_dynamic` is required if the field is
    filterable. Takes precedence over `:field_dynamic` for filtering.
