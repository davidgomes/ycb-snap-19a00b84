defprotocol Flop.Schema do
  ...
  - `filter` is called to filter by the field. It receives the Ecto query, the
    ...
    Required for a filterable custom field unless `field_dynamic` is set. When
    both are set, `filter` is used for filtering.
  - `field_dynamic` receives an options keyword list and returns an
    `Ecto.Query.dynamic_expr`. It receives neither the query nor the direction
    or operator. Flop applies the order direction to the expression when the
    field is used for sorting, and applies the filter operator to the same
    expression when the field is used for filtering and no `filter` function is
    configured. A custom field needs `field_dynamic` to be sortable. A
    filterable custom field needs `filter` or `field_dynamic`.
  ...
  - `:filter` - A module/function/options tuple referencing a custom filter
    ...
    Required if the field is filterable and `:field_dynamic` is not set. When
    both are set, this function is used for filtering.
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    when filtering if no `:filter` function is configured. Required if the field
    is sortable. Also sufficient for a filterable field.
