defprotocol Flop.Schema do
  ...
  To make a custom field sortable, set the `:field_dynamic` option to a tuple
  `{mod :: module, function :: atom, opts :: keyword}`. The referenced function
  receives the options keyword list and must return the field expression as an
  `Ecto.Query.dynamic_expr`. Flop applies the order directions on the returned
  expression. The `extra_opts` are merged into the options as well. Custom
  fields cannot be used as order fields with cursor-based pagination.
  ...
  If your custom filter or field dynamic requires certain named bindings, you
  can use the `:bindings` option to specify them. Then, using
  `Flop.with_named_bindings/4`, these bindings can be conditionally added to
  your query based on the filter and order parameters.
  ...
  - `:sortable` (required) - A list of fields that can be used for sorting.
    ...
    fields that set the `:field_dynamic` option.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    applies the order directions on the expression. Required if the field is
    sortable.
  ...
  - `:bindings` - If the custom filter or field dynamic function requires
    certain named bindings to be present in the Ecto query, you can specify
    them here. These bindings will be conditionally added by
    `Flop.with_named_bindings/4` if the field is used in a filter or in the
    order.
