defprotocol Flop.Schema do
  ...
  - `filter` receives three arguments: the Ecto query, the Flop filter, and an
    options keyword list. It returns the updated query. This option is required
    to make a custom field filterable.
  - `field_dynamic` receives an options keyword list and returns the field
    itself as an `Ecto.Query.dynamic_expr`. Flop applies the ordering clauses
    on the expression it returns. This option is required to make a custom
    field sortable.
  ...
  - `:sortable` (required) - A list of fields that can be used for sorting.
    ...
    fields, and custom fields that set the `:field_dynamic` option.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    applies the ordering clauses on the expression. Required if the field is
    sortable.
  ...
  - `:bindings` - If the custom filter function or the field dynamic requires
    ...
    `Flop.with_named_bindings/4` if the field is used for filtering or
    ordering.
