defprotocol Flop.Schema do
  ...
  - `filter` is called to filter by the field. It receives the Ecto query, the
    Flop filter and an options keyword list, and returns the updated query. Use
    this when the built-in operators cannot express the filter. A custom field
    needs either `filter` or `field_dynamic` to be filterable.
  - `field_dynamic` is called to order by the field, and to filter by the field
    when `filter` is not set. It receives an options keyword list and returns an
    `Ecto.Query.dynamic_expr`. For ordering, Flop applies the order direction to
    the expression. For filtering, Flop applies the built-in operators to it.
    It receives neither the query nor the direction. A custom field needs it to
    be sortable.
  ...
  - `:field_dynamic` - A module/function/options tuple referencing a function
    ...
    the order direction to the expression when sorting, and the built-in filter
    operators when filtering if `:filter` is not set. Required if the field is
    sortable. Also sufficient to make the field filterable.
