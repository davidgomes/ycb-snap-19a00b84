defmodule ObanDoctor.Check.Worker.UniquenessMissingStates do
  ...
  Named state groups (`:incomplete`, `:scheduled`, `:successful`) are expanded to
  their member states before comparison. The `:all` group is handled by
  ...
  defp normalize_states(group) when is_map_key(@state_groups, group), do: @state_groups[group]
