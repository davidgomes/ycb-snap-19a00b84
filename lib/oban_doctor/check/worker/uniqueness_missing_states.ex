defmodule ObanDoctor.Check.Worker.UniquenessMissingStates do
  ...
  Named state groups are expanded before checking. `:incomplete` and `:successful`
  include every recommended state, while `:scheduled` only covers `[:scheduled]`.
  The `:all` group is handled by `ObanDoctor.Check.Worker.StateGroupUsage`.
  ...
  Bad - the `:scheduled` group only includes the scheduled state:
