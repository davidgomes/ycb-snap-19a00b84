defmodule ObanDoctor.Check.Worker.UniquenessMissingStates do
  ...
  **Why this matters:** Missing `:retryable` is a common issue. When a job fails and enters
  the retryable state (with backoff), a new job with the same unique key can be enqueued,
  causing duplicates. This is especially problematic during deployments or pod scaling.

  This check does NOT flag named state groups (`:incomplete`, `:scheduled`, `:successful`)
  as these are intentional Oban patterns. `:all` is reported by `StateGroupUsage`.
  ...
  defp missing_recommended_states?(%{unique: unique}) do
    ...
    # :incomplete, :scheduled, and :successful are intentional.
    # Named groups are only valid as atoms or single-element lists
    # (states: :incomplete or states: [:incomplete]). A list such as
    # [:available, :scheduled, :executing] is not a named group, even though
    # :scheduled is both a state name and a group name.
