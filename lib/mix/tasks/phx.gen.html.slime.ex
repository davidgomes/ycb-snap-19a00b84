defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  ...
  This generator accepts the same options as `phx.gen.html`, such as
  `--web`, `--context-app`, `--no-context`, `--no-schema`, `--table`
  and `--binary-id`. Read the documentation for `phx.gen.html` for more
  information.
  """
  use Mix.Task
  ...
  defp slime_files(%Context{schema: schema, context_app: context_app}) do
    templates_path = Path.join([Mix.Phoenix.web_path(context_app), "templates", to_string(schema.web_path), schema.singular])
    extension = PhoenixSlime.ConfiguredExtension.file_extension

    [
      {:eex, "edit.html.eex",       Path.join(templates_path, "edit.html.#{extension}")},
      {:eex, "form.html.eex",       Path.join(templates_path, "form.html.#{extension}")},
      {:eex, "index.html.eex",      Path.join(templates_path, "index.html.#{extension}")},
      {:eex, "new.html.eex",        Path.join(templates_path, "new.html.#{extension}")},
      {:eex, "show.html.eex",       Path.join(templates_path, "show.html.#{extension}")},
    ]
  end
