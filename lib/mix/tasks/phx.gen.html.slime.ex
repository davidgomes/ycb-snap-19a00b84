defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  ...
  defp prompt_for_conflicts(context) do
    ...
    |> Kernel.++(slime_files_to_be_generated(context))
  ...
  @doc false
  def slime_files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    web_prefix = Mix.Phoenix.web_path(context_app)
    web_path = to_string(schema.web_path)
    extension = PhoenixSlime.ConfiguredExtension.file_extension

    for name <- ~w(edit form index new show) do
      {:eex, "#{name}.html.eex", Path.join([web_prefix, "templates", web_path, schema.singular, "#{name}.html.#{extension}"])}
    end
  end

  @doc false
  def copy_new_files(%Context{} = context, paths, binding) do
    Mix.Phoenix.copy_from(paths, "priv/templates/phx.gen.html", binding, files_to_be_generated(context))
    Mix.Phoenix.copy_from(slime_paths(), "priv/templates/phx.gen.html.slime", binding, slime_files_to_be_generated(context))
