defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  ...
  The generated resource will contain:
    ...
    * test files for the generated context and controller

  The generated context and schema can be skipped with `--no-context`
  and `--no-schema`. Read the documentation for `phx.gen.html` for more
  information on the available options and attributes.
  """
  ...
  defp slime_files(%Context{schema: schema, context_app: context_app}) do
    ...
    for template <- ~w(edit form index new show) do
      {:eex, "#{template}.html.eex", Path.join(templates_path, "#{template}.html.#{extension}")}
    end
  end

  @doc false
  def copy_new_files(%Context{} = context, binding) do
    Mix.Phoenix.copy_from Mix.Phoenix.generator_paths(), "priv/templates/phx.gen.html", binding, phoenix_files(context)
