defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  ...
  def slime_files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    web_prefix = Mix.Phoenix.web_path(context_app)
    template_path = Path.join([web_prefix, "templates", to_string(schema.web_path), schema.singular])
    extension = PhoenixSlime.ConfiguredExtension.file_extension

    for template <- ~w(edit form index new show) do
      {:eex, "#{template}.html.eex", Path.join(template_path, "#{template}.html.#{extension}")}
    end
  end
