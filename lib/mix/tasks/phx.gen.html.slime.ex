defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  ...
  `--context-app`. Read the documentation for `phx.gen.html` for more
  information.
  """
  ...
  defp slime_files(%Context{schema: schema, context_app: context_app}) do
    web_prefix = Mix.Phoenix.web_path(context_app)
    template_prefix = Path.join([web_prefix, "templates", to_string(schema.web_path), schema.singular])
    extension = PhoenixSlime.ConfiguredExtension.file_extension

    [
      {:eex, "edit.html.eex",       Path.join(template_prefix, "edit.html.#{extension}")},
      {:eex, "form.html.eex",       Path.join(template_prefix, "form.html.#{extension}")},
      {:eex, "index.html.eex",      Path.join(template_prefix, "index.html.#{extension}")},
      {:eex, "new.html.eex",        Path.join(template_prefix, "new.html.#{extension}")},
      {:eex, "show.html.eex",       Path.join(template_prefix, "show.html.#{extension}")},
    ]
  end
