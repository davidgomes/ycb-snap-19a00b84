defmodule Mix.Tasks.Phx.Gen.Html.Slime do
  @shortdoc "Generates controller, views, and context for an HTML resource with Slime templates"
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      ...
      {:eex, "edit.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "edit.html.#{extension}"])},
      {:eex, "form.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "form.html.#{extension}"])},
      {:eex, "index.html.eex",      Path.join([web_prefix, "templates", web_path, schema.singular, "index.html.#{extension}"])},
      {:eex, "new.html.eex",        Path.join([web_prefix, "templates", web_path, schema.singular, "new.html.#{extension}"])},
      {:eex, "show.html.eex",       Path.join([web_prefix, "templates", web_path, schema.singular, "show.html.#{extension}"])},
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    {template_files, other_files} = Enum.split_with(files, fn {_, source, _} ->
      String.ends_with?(source, ".html.eex")
    end)

    Mix.Phoenix.copy_from(paths, "priv/templates/phx.gen.html", binding, other_files)
    Mix.Phoenix.copy_from(slime_paths(), "priv/templates/phx.gen.html.slime", binding, template_files)
