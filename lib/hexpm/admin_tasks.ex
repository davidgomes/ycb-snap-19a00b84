defmodule Hexpm.AdminTasks do
  ...
  - `opts` - Options:
    ...
    - `:reason` - When given, the user is emailed that their account (and any
      deleted packages) was removed and why
  ...
  ## Examples
      ...
      iex> AdminTasks.remove_user("spammer", reason: "Publishing malware")
      :ok
  """
  ...
  def remove_user(username, opts \\ []) do
    ...
    with {:ok, user} <- find_user(username) do
      if Keyword.get(opts, :delete_packages, false) do
        ...
        |> case do
          {:ok, deleted} ->
            Enum.each(deleted, &run_package_removal_side_effects(&1, reason))
  ...
  defp notify_account_removed(user, reason) do
    Emails.account_removed(user, reason)
    |> Mailer.deliver!()
  ...
  def remove_package(repo, package_name, opts \\ []) do
    with {:ok, package} <- find_package(repo, package_name) do
      ...
      run_package_removal_side_effects(removed, Keyword.get(opts, :reason))
      :ok
    end
  end
  ...
  defp remove_package_db(package) do
    ...
    owners =
      package_owners
      |> Repo.preload(user: [:emails, organization: [organization_users: [user: :emails]]])
      |> Enum.map(& &1.user)
    ...
    Repo.delete!(package)
    {releases, package, owners}
  end

  defp run_package_removal_side_effects({releases, package, owners}, reason) do
