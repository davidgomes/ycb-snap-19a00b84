defmodule Hexpm.AdminTasks do
  ...
  - `opts` - Options:
    ...
    - `:reason` - When given, the user is emailed that their account was
      removed along with this reason and any packages that were deleted
      (default: `nil`, no email is sent)
  ...
  ## Examples
      ...
      iex> AdminTasks.remove_user("spammer", delete_packages: true, reason: "Publishing malware")
      :ok
  """
      ...
      if Keyword.get(opts, :delete_packages, false) do
        ...
        |> case do
          {:ok, deleted} ->
            ...
            package_names = Enum.map(deleted, fn {_releases, package} -> package.name end)
            notify_user_removed(user, reason, package_names)
            :ok
        ...
        with :ok <- Users.delete(user, audit: AuditLogs.admin(), notify: false) do
          notify_user_removed(user, reason, [])
        end
      end
    end
  end

  defp notify_user_removed(_user, nil, _packages), do: :ok

  defp notify_user_removed(user, reason, packages) do
  ...
  - `opts` - Options:
    - `:reason` - When given, the package owners are emailed that the package
      was removed along with this reason (default: `nil`, no email is sent)
  ...
  ## Examples
      ...
      iex> AdminTasks.remove_package("hexpm", "malicious_pkg", reason: "Contains malware")
      :ok
  """
