defmodule Hexpm.AdminTasks do
  ...
  - `opts` - Options:
    ...
    - `:reason` - When given, the user is emailed that their account was
      removed and why, including any packages removed with it (default: `nil`,
      no email is sent)
      ...
      if Keyword.get(opts, :delete_packages, false) do
        ...
        |> case do
          {:ok, deleted} ->
            ...
            notify_account_removed(user, reason, package_names)
            :ok
  ...
  defp notify_account_removed(_user, nil, _packages), do: :ok

  defp notify_account_removed(user, reason, packages) do
    if User.email(user, :primary) do
      Emails.account_removed(user, reason, Enum.sort(packages))
      |> Mailer.deliver!()
    end
  ...
  - `opts` - Options:
    - `:reason` - When given, the package owners are emailed that the package
      was removed and why (default: `nil`, no email is sent)
