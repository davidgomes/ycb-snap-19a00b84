defmodule Hexpm.Repository.Packages do
  ...
  def attach_latest_releases(packages) do
    ...
    latest = Release.latest_of_package(only_stable: true, unstable_fallback: true)
    ...
    releases =
      from(
        ...
        on: true,
        select: {p.id, type(r.version, :string), r.inserted_at}
      )
      |> Repo.all()
      |> Map.new(fn {package_id, version, inserted_at} ->
        {package_id, %Release{version: version, inserted_at: inserted_at}}
      end)
