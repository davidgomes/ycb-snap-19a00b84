...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  def unregister_completed_entry(%UploadConfig{} = conf, entry_ref) do
    case get_entry_by_ref(conf, entry_ref) do
      # A writer failure keeps the entry until it is explicitly cancelled.
      %UploadEntry{} = entry ->
        if entry_failed?(conf, entry_ref) do
          conf
        else
          drop_entry(conf, entry)
        end
  ...
  @doc false
  def entry_failed?(%UploadConfig{} = conf, entry_ref) do
  ...
  def fail_entry(%UploadConfig{} = conf, entry_ref, reason) do
    conf =
      if {entry_ref, reason} in conf.errors do
        conf
      else
        put_error(conf, entry_ref, reason)
      end

    %{conf | entry_refs_to_pids: Map.put(conf.entry_refs_to_pids, entry_ref, @failed)}
  end
