...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  def fail_entry(%UploadConfig{} = conf, entry_ref, reason) do
    # the entry may already be gone, for example when a progress callback
    # cancelled it, in which case the failure report is stale
    case get_entry_by_ref(conf, entry_ref) do
      %UploadEntry{} ->
        conf = %{conf | entry_refs_to_pids: Map.put(conf.entry_refs_to_pids, entry_ref, @failed)}
