...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  def unregister_completed_entry(%UploadConfig{} = conf, entry_ref) do
    case {get_entry_by_ref(conf, entry_ref), Map.fetch(conf.entry_refs_to_pids, entry_ref)} do
      {%UploadEntry{}, {:ok, @failed}} -> conf
      {%UploadEntry{} = entry, _} -> drop_entry(conf, entry)
      # the entry may already be gone, e.g. a failed entry cancelled before its channel exited
      {nil, _} -> conf
    end
  end
