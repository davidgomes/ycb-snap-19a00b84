...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  def unregister_completed_entry(%UploadConfig{} = conf, entry_ref) do
    case get_entry_by_ref(conf, entry_ref) do
      %UploadEntry{} = entry ->
        if failed_entry?(conf, entry), do: conf, else: drop_entry(conf, entry)
  ...
  def fail_entry(%UploadConfig{} = conf, entry_ref, reason) do
    case get_entry_by_ref(conf, entry_ref) do
      %UploadEntry{} ->
        ...
        new_pids = Map.put(conf.entry_refs_to_pids, entry_ref, @failed)
        errors = if pair in conf.errors, do: conf.errors, else: conf.errors ++ [pair]
        %{conf | entry_refs_to_pids: new_pids, errors: errors}
  ...
  defp failed_entry?(%UploadConfig{} = conf, %UploadEntry{ref: ref}) do
    Map.get(conf.entry_refs_to_pids, ref) == @failed
  end
