...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  @invalid :invalid
  # entries whose writer failed are retained until they are cancelled or replaced
  @failed :failed
  ...
  def unregister_completed_entry(%UploadConfig{} = conf, entry_ref) do
    case {get_entry_by_ref(conf, entry_ref), Map.get(conf.entry_refs_to_pids, entry_ref)} do
      ...
      # the entry may have been cancelled after its writer failed
      {nil, _status} -> conf
    end
  end
  ...
  def fail_entry(%UploadConfig{} = conf, entry_ref, reason) do
    case get_entry_by_ref(conf, entry_ref) do
      %UploadEntry{} ->
        error = {entry_ref, reason}
        errors = if error in conf.errors, do: conf.errors, else: conf.errors ++ [error]
        pids = Map.put(conf.entry_refs_to_pids, entry_ref, @failed)
        %{conf | errors: errors, entry_refs_to_pids: pids}
