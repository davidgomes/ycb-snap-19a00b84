...
defmodule Phoenix.LiveView.UploadConfig do
  ...
  defp too_many_files?(%UploadConfig{entries: entries, max_entries: max} = conf) do
    counted_consumed(conf) + length(entries) > max
  end

  defp counted_consumed(%UploadConfig{max_entries_mode: :total, consumed_entries: count}),
    do: count

  defp counted_consumed(%UploadConfig{}), do: 0

  @doc false
  def remaining_entries(%UploadConfig{max_entries: max} = conf) do
    max(max - counted_consumed(conf), 0)
  end
  ...
  @doc false
  def consume_entry(%UploadConfig{max_entries_mode: :total} = conf, %UploadEntry{} = entry) do
    conf
