defmodule Phoenix.LiveView.Channel do
  ...
  def release_upload_name(name) do
    send(self(), {@prefix, :release_upload_name, name})
  end
  ...
  def handle_info({:DOWN, _, :process, pid, reason} = msg, %{socket: socket} = state) do
    case Map.fetch(state.upload_pids, pid) do
      {:ok, {ref, entry_ref, cid}} ->
        # `:shutdown` is the channel exit when join/3 returns an error, which
        # is how a writer `init/1` failure stops the upload channel.
  ...
  def handle_info(%Message{topic: topic, event: "progress"} = msg, %{topic: topic} = state) do
    ...
    new_state =
      write_socket(state, cid, msg.ref, fn socket, _ ->
        ...
        new_socket =
          if entry && upload_conf.progress_event do
            invoke_upload_progress(upload_conf, entry, new_socket)
          else
            new_socket
          end
  ...
  def handle_info({@prefix, :report_writer_error, channel_pid, reason}, state) do
    case state.upload_pids do
      %{^channel_pid => {ref, entry_ref, cid}} ->
        new_state =
          write_socket(state, cid, nil, fn socket, _ ->
            {fail_writer_entry(socket, ref, entry_ref, reason), {:ok, nil, state}}
          end)
  ...
  def handle_info({@prefix, :release_upload_name, name}, state) do
  ...
  defp unregister_upload(state, ref, entry_ref, cid) do
    write_socket(state, cid, nil, fn socket, _ ->
      case Upload.fetch_upload_by_ref(socket, ref) do
        ...
        {:ok, conf} ->
          new_state =
            if release_upload_name_on_exit?(conf, entry_ref) do
              drop_upload_name(state, conf.name)
            else
              state
            end

          {Upload.unregister_completed_entry_upload(socket, conf, entry_ref),
           {:ok, nil, new_state}}
      end
    end)
  end

  # Drop the name only when this exit actually removes the last entry.
  # Writer failures stay registered until cancel, and a missing entry was
  # already removed (for example from the progress callback).
  defp release_upload_name_on_exit?(conf, entry_ref) do
    case UploadConfig.get_entry_by_ref(conf, entry_ref) do
      nil ->
        false

      _entry ->
        not UploadConfig.entry_failed?(conf, entry_ref) and
          Enum.all?(conf.entries, &(&1.ref == entry_ref))
    end
  end

  defp fail_writer_entry(socket, ref, entry_ref, reason) do
    conf = Upload.get_upload_by_ref!(socket, ref)
    socket = Upload.fail_entry(socket, conf, entry_ref, {:writer_failure, reason})
    conf = Upload.get_upload_by_ref!(socket, ref)
    entry = UploadConfig.get_entry_by_ref(conf, entry_ref)

    if entry && conf.progress_event do
      invoke_upload_progress(conf, entry, socket)
    else
      socket
    end
  end

  defp invoke_upload_progress(upload_conf, entry, socket) do
    event = upload_conf.progress_event

    case event.(upload_conf.name, entry, socket) do
      {:noreply, %Socket{} = socket} ->
        if socket.redirected do
          flash = Utils.changed_flash(socket)
          send(socket.root_pid, {@prefix, :redirect, socket.redirected, flash})
          %{socket | redirected: nil}
        else
          socket
        end
