defmodule Phoenix.LiveView.Channel do
  ...
  def handle_info({:DOWN, _, :process, pid, reason} = msg, %{socket: socket} = state) do
    case Map.fetch(state.upload_pids, pid) do
      {:ok, {ref, entry_ref, cid}} ->
        # :shutdown is how the upload channel exits when its join fails after
        # registering the entry, such as on a writer init/1 error
  ...
  def handle_info({@prefix, :drop_upload_name, info}, state) do
    ...
    # the name may have been re-registered for a newer upload in the meantime
    new_state =
  ...
  defp unregister_upload(state, ref, entry_ref, cid) do
    write_socket(state, cid, nil, fn socket, _ ->
      case Upload.fetch_upload_by_ref(socket, ref) do
        ...
        # a writer failure progress callback may cancel the entry and disallow
        # the upload before the upload channel exit is handled
        :error ->
          {socket, {:ok, nil, state}}
      end
    end)
    ...
    write_socket(state, cid, nil, fn socket, _ ->
      ...
      case Upload.register_entry_upload(socket, conf, pid, entry_ref) do
        {:ok, new_socket, entry} ->
          ...
          # monitor before replying, as the upload channel may exit right after
          # the reply, such as on a writer init/1 error, which must not be :noproc
