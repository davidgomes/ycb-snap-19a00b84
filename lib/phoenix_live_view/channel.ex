      ...
      %{^channel_pid => {ref, entry_ref, cid}} ->
        new_state =
          write_socket(state, cid, nil, fn socket, _ ->
            ...
            new_socket =
              Upload.fail_entry_upload(socket, upload_conf, entry_ref, {:writer_failure, reason})
  ...
  defp unregister_upload(state, ref, entry_ref, cid) do
    write_socket(state, cid, nil, fn socket, _ ->
      case Upload.fetch_upload_by_ref(socket, ref) do
        ...
        # the progress callback may cancel the failed entry and disallow the upload
        # before the upload channel's :DOWN is handled
        :error ->
          {socket, {:ok, nil, state}}
      end
    end)
    ...
    write_socket(state, cid, nil, fn socket, _ ->
      ...
      case Upload.register_entry_upload(socket, conf, pid, entry_ref) do
        {:ok, new_socket, entry} ->
          ...
          # monitor before replying, otherwise a channel that exits right after being
          # acknowledged (writer init/1 failure) may be dead by the time we monitor it
