defmodule Phoenix.LiveView.Upload do
  ...
  def cancel_upload(socket, name, entry_ref) do
    ...
    case UploadConfig.get_entry_by_ref(upload_config, entry_ref) do
      %UploadEntry{} = entry ->
        ...
        # entries without an upload channel are dropped right away, so there
        # is no channel exit that would release the upload name
  ...
  @doc """
  Fails an entry, retaining it with the given error until it is cancelled.
  """
