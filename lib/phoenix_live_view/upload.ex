defmodule Phoenix.LiveView.Upload do
  ...
  def cancel_upload(socket, name, entry_ref) do
    ...
    case UploadConfig.get_entry_by_ref(upload_config, entry_ref) do
      %UploadEntry{} = entry ->
        ...
        # entries without an upload channel, such as failed ones, are dropped right
        # away, so no channel exit will release the upload name for them
  ...
  @doc """
  Fails an entry upload, retaining the entry with the given error.
  """
