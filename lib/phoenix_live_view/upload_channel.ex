  ...
  def join(_topic, auth_payload, socket) do
    ...
    with {:ok, %{pid: pid, ref: ref, cid: cid}} <- Static.verify_token(socket.endpoint, token),
      ...
      case writer.init(writer_opts) do
        ...
        {:error, reason} ->
          # the entry is already registered with the LiveView, so the failure must be
          # reported just like a write_chunk/2 or close/2 failure to retain the entry
          :ok = Channel.report_writer_error(pid, reason)
