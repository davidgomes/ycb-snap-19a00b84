defmodule Phoenix.LiveView.UploadChannel do
  ...
  def join(_topic, auth_payload, socket) do
    ...
    with {:ok, %{pid: pid, ref: ref, cid: cid}} <- Static.verify_token(socket.endpoint, token),
         ...
         {writer, writer_opts} <- config.writer do
      case writer.init(writer_opts) do
        ...
        # The entry is already registered; report the failure before the
        # channel shuts down so the LiveView retains it like other errors.
        {:error, reason} ->
