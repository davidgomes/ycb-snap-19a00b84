defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  @spec do_reserve_available_jobs(module, repo, schema, integer, Keyword.t()) ::
          {integer, [job]}
  ...
  # MySQL supports neither CTEs in UPDATE statements nor RETURNING, so the available
  # jobs are locked, updated and re-read within a single transaction.
  ...
  # Applies `updates` to the single job matched by `query`, returning the updated job.
  # MySQL has no RETURNING clause, so the job is re-read after a successful update.
  @spec update_job(repo, job, Ecto.Query.t(), Keyword.t()) :: {integer, [job]}
  defp update_job(repo, job = %schema{}, query, updates) do
