defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  def update_job_in_progress(repo, job = %_schema{}, now, timeout_ms) do
  ...
  def job_failed(repo, job = %_schema{}, now, retry_timeout_ms) do
    updates =
  ...
  # MyXQL cannot return updated rows from UPDATE, and does not support the
  # UPDATE ... FROM (CTE) form used on Postgres.
  defp do_update_job_in_progress(Ecto.Adapters.Postgres, repo, job = %schema{}, now, timeout_ms) do
    repo.update_all(
      Query.from(
        j in schema,
        where: j.id == ^job.id,
    ...
    {:ok, result} =
      repo.transaction(fn ->
        ...
        if ids == [] do
          ...
          jobs = repo.all(Query.from(j in schema, where: j.id in ^ids, select: j))
          {count, jobs}
        end
      end)
