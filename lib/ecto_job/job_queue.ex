  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  def reserve_available_jobs(repo, schema, demand, now = %DateTime{}, timeout_ms) do
    ...
    case repo.__adapter__() do
      ...
      _adapter ->
        update_locked_jobs(repo, schema, available_jobs(schema, demand), updates)
    end
  end
  ...
  # Equivalent of `repo.update_all(Query.select(query, [j], j), set: updates)`
  # for adapters that don't support `UPDATE ... RETURNING` (e.g. MySQL).
  ...
  defp update_all_returning(repo, schema, query, updates) do
    case repo.__adapter__() do
      ...
      _adapter ->
        lock_query = Query.from(j in query, lock: "FOR UPDATE", select: %{id: j.id})
        update_locked_jobs(repo, schema, lock_query, updates)
    end
  end

  # `ids_query` must select `%{id: id}` and lock the selected rows, so they cannot
  # change between the select, the update and the final read.
  @spec update_locked_jobs(repo, schema, Ecto.Query.t(), Keyword.t()) :: {integer, [job]}
  defp update_locked_jobs(repo, schema, ids_query, updates) do
    {:ok, result} =
      repo.transaction(fn ->
        case Enum.map(repo.all(ids_query), & &1.id) do
          [] ->
            {0, []}

          ids ->
            {count, _} =
              repo.update_all(Query.from(j in schema, where: j.id in ^ids), set: updates)
