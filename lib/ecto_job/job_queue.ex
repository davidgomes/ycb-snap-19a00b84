defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  # Postgres supports `RETURNING` via `select` in `update_all`, so the updated
  # rows can be fetched atomically as part of the update itself.
  ...
  # MySQL/MyXQL doesn't support `RETURNING`, so the update and a follow up select
  # are combined in a transaction to fetch the updated row.
  ...
  # MySQL doesn't support CTEs combined with `RETURNING` on `update_all`, so the
  # available job ids are selected first (locking them with `FOR UPDATE SKIP LOCKED`),
  # then reserved and reloaded within the same transaction.
  defp do_reserve_available_jobs(
         Ecto.Adapters.MyXQL,
         repo,
         schema,
    ...
    {:ok, {count, results}} =
      repo.transaction(fn ->
        ...
        {count, nil} =
          repo.update_all(
            Query.from(j in schema, where: j.id in ^ids),
            set: [
              state: "RESERVED",
        ...
        results =
          Query.from(j in schema, where: j.id in ^ids, select: j)
          |> repo.all()
