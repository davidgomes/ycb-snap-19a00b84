defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  @spec do_reserve_available_jobs(module, repo, schema, integer, Keyword.t()) :: {integer, [job]}
  ...
  # Without `UPDATE ... RETURNING` (e.g. MySQL), the batch is locked, updated and
  # read back within a single transaction.
  defp do_reserve_available_jobs(_adapter, repo, schema, demand, updates) do
    {:ok, result} =
      repo.transaction(fn ->
        case Enum.map(repo.all(available_jobs(schema, demand)), & &1.id) do
          [] ->
            {0, []}

          ids ->
            {count, _} = repo.update_all(Query.where(schema, [j], j.id in ^ids), set: updates)
            ...
            jobs =
              repo.all(
                Query.from(
                  j in schema,
                  where: j.id in ^ids,
                  order_by: [asc: j.priority, asc: j.schedule, asc: j.id]
                )
              )
  ...
  # Applies `updates` to `job` if it still matches `query`, returning `{count, updated_jobs}`.
  @spec update_job(repo, job, Ecto.Query.t(), Keyword.t()) :: {integer, [job]}
  defp update_job(repo, job, query, updates) do
    do_update_job(repo.__adapter__(), repo, job, query, updates)
  end

  @spec do_update_job(module, repo, job, Ecto.Query.t(), Keyword.t()) :: {integer, [job]}
  defp do_update_job(Ecto.Adapters.Postgres, repo, _job, query, updates) do
    repo.update_all(Query.select(query, [j], j), set: updates)
  end

  # The updated row stays locked until the transaction commits, so reading it back
  # returns exactly what was written.
  defp do_update_job(_adapter, repo, job = %schema{}, query, updates) do
    {:ok, result} =
      repo.transaction(fn ->
        case repo.update_all(query, set: updates) do
          {0, _} -> {0, []}
          {count, _} -> {count, [repo.get!(schema, job.id)]}
        end
      end)
