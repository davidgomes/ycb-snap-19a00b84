defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  # MySQL supports neither `UPDATE ... RETURNING` nor `LIMIT` inside an `IN` subquery,
  # so the jobs are locked, updated and re-read within a single transaction.
  ...
  # Applies `updates` to the job matched by `query`, returning `{count, updated_jobs}`.
  # MySQL has no `UPDATE ... RETURNING`, so the updated job is re-read by id instead.
