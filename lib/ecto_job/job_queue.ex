defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [
      ...
      Options:
       ...
       - `:notify` (string): payload to use for Postgres notification upon job completion (ignored with MySQL)
      """
  ...
  defp update_all_returning(repo, schema, query, updates) do
    case repo.__adapter__() do
      Ecto.Adapters.MyXQL ->
        ids_query = query |> Query.select([j], %{id: j.id}) |> Query.lock("FOR UPDATE")
        update_locked_jobs(repo, schema, ids_query, updates)
  ...
  # MySQL has no RETURNING clause, so the selected rows are locked, updated and then
  ...
  defp update_locked_jobs(repo, schema, ids_query, updates) do
    {:ok, result} =
      repo.transaction(fn ->
        ids = ids_query |> repo.all() |> Enum.map(& &1.id)
