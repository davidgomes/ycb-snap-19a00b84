defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  # MyXQL cannot return updated rows from UPDATE, so read them back in the same transaction.
  ...
  # MySQL has no UPDATE ... RETURNING and does not support this CTE update shape.
  defp do_reserve_available_jobs(
         Ecto.Adapters.MyXQL,
         repo,
         schema,
