defmodule EctoJob.JobQueue do
  ...
  * `:timestamps_opts` - (_optional_) Configures the timestamp fields for the schema (See `Ecto.Schema.timestamps/1`)
  ...
  defp do_reserve_available_jobs(_adapter, repo, schema, demand, updates) do
    {:ok, result} =
      repo.transaction(fn ->
        ...
        update_jobs_by_ids(repo, schema, ids, updates)
      end)
  ...
  # Runs `update_all` and returns the updated jobs.
  # Adapters without `UPDATE ... RETURNING` support (MySQL) lock the matching rows,
  # update them by id and read them back within a single transaction.
  @spec update_all_returning(repo, schema, Ecto.Query.t(), Keyword.t()) :: {integer, [job]}
  defp update_all_returning(repo, schema, query, updates) do
  ...
  @spec update_jobs_by_ids(repo, schema, [integer], Keyword.t()) :: {integer, [job]}
  defp update_jobs_by_ids(_repo, _schema, [], _updates), do: {0, []}

  defp update_jobs_by_ids(repo, schema, ids, updates) do
