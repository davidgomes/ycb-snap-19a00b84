defmodule SpiderMan do
  ...
  * [ItemProcessor](SpiderMan.Component.ItemProcessor.html): Store items.
  ...
  @spec stats(spider) :: [
          ...
          throughputs: [component_throughput]
        ]
  def stats(spider) do
    components =
  ...
  @doc "check whether the spider has no pending or failed tasks"
  ...
  @doc """
  fetch the throughput infos of each component for a spider, ordered by message flow:
  `Downloader` -> `Spider` -> `ItemProcessor`.

  `tps` is the number of successful events per second of processing time.
  """
  @spec throughput(spider | :ets.tid()) :: [component_throughput]
