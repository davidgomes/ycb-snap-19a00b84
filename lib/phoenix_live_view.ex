  ...
  *Note*: You must also place the `Phoenix.LiveView.Router.fetch_live_flash/2`
  ...
  ## Options
    ...
    * `:max_entries_mode` - Controls how `:max_entries` is counted. `:selected`
      only counts the entries currently selected, so consuming an entry frees
      capacity for another. `:total` also counts the entries already consumed
      over the lifetime of the upload configuration, which is useful to limit
      auto uploads that are consumed as they complete. Once all entries have been
      consumed or cancelled, call `allow_upload/3` again to reset a `:total` limit.
