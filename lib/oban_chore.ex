defmodule ObanChore do
  ...
  @doc """
  Lists the most recent finished (completed, discarded, cancelled) jobs for a given worker module.

  Returns up to `limit` `%Oban.Job{}` structs, newest first, with the state converted to an atom.
  """
  def list_history_jobs(worker_module, limit \\ 20, oban_name \\ Oban) do
    config = Oban.config(oban_name)
    repo = config.repo
