defmodule ObanChore do
  ...
  @doc """
  Lists the most recent finished (completed, cancelled, discarded) jobs for a given worker module.

  Jobs are ordered from newest to oldest and at most `limit` jobs are returned.

  Returns a list of `%Oban.Job{}` structs with the state converted to an atom.
  """
  def list_job_history(worker_module, oban_name \\ Oban, limit \\ 20) do
    config = Oban.config(oban_name)
    repo = config.repo
