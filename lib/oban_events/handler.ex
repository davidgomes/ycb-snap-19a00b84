defmodule ObanEvents.Handler do
  ...
  the event name and an `ObanEvents.Event` struct.
  ...
  Receives the event name (atom) and an `ObanEvents.Event` struct carrying the
  event data (map with string keys) plus metadata.
  Should process the event and return an ok/error tuple.

  ## Parameters

  - `event_name`: Atom representing the event (e.g., `:user_created`)
  - `event`: `ObanEvents.Event` struct with `data`, `event_id`, `idempotency_key`,
