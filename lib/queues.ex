defmodule Oban.Console.Queues do
  ...
  def pause_queues(names) when is_list(names), do: Enum.map(names, &pause_queues/1)

  def pause_queues(name) when is_binary(name) do
    :ok = Oban.pause_queue(queue: name)
  ...
  def resume_queues(names) when is_list(names), do: Enum.map(names, &resume_queues/1)

  def resume_queues(name) when is_binary(name) do
    :ok = Oban.resume_queue(queue: name)
