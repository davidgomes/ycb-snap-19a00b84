defmodule Paginator do
  ...
  function returning an `Ecto.Query.dynamic/2` expression. The expression is used
  to filter the query, while `name` is the key used in the cursor, so a custom
  `:fetch_cursor_value_fun` is needed to compute its value from the returned records.

      query =
        from(
          p in Post,
          order_by: [desc: fragment("char_length(?)", p.title), asc: p.id],
          select: p
        )
