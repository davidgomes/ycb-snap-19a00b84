defmodule Paginator do
  ...
  ## Options
    ...
    * `:cursor_fields` - The fields with sorting direction used to determine the
    ...
    directions are `:asc`, `:asc_nulls_last`, `:asc_nulls_first`, `:desc`, `:desc_nulls_first`
    and `:desc_nulls_last`. `:asc` and `:desc` follow the Postgres defaults, i.e. they are
    equivalent to `:asc_nulls_last` and `:desc_nulls_first`. The last cursor field must not
    be nullable.
