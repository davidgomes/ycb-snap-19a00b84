defmodule Paginator do
  ...
  ## Options
    ...
    * `:cursor_fields` - The fields with sorting direction used to determine the
    ...
    When you use named bindings in your query they can also be provided. The supported sorting
    directions are `:asc`, `:asc_nulls_first`, `:asc_nulls_last`, `:desc`, `:desc_nulls_first`
    and `:desc_nulls_last`. `:asc` and `:desc` follow the PostgreSQL default of sorting `NULL`
    values last and first respectively.
