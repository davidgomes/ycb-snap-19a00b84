defmodule Paginator do
  ...
  ## Options
    ...
    * `:sort_direction` - The direction used for sorting. Defaults to `:asc`.
    ...
    Supported directions are `:asc`, `:asc_nulls_last`, `:asc_nulls_first`,
    `:desc`, `:desc_nulls_first`, and `:desc_nulls_last`. `:asc` follows the
    Postgres default of nulls last, and `:desc` follows nulls first.
