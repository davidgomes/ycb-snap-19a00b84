defmodule ErrorTracker.Telemetry do
  ...
  * `[:error_tracker, :error, :muted]`: is emitted when an error is muted on the
  UI.

  * `[:error_tracker, :error, :unmuted]`: is emitted when an error is unmuted on
  the UI.
  ...
  * `[:error_tracker, :occurrence, :new]`: is emitted when a new occurrence is
  ...
  Muted errors keep storing their occurrences, but no events are emitted for
  them: neither the error events nor the occurrence ones. This allows you to
  ignore noisy errors on your notifications and integrations.
