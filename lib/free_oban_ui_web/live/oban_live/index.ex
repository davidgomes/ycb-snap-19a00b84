defmodule FreeObanUiWeb.ObanLive.Index do
  ...
  def handle_event("bulk_action", %{"action" => action}, socket) do
    count = MapSet.size(socket.assigns.selected_jobs)
    ...
    {:noreply,
     ...
     |> put_flash(:info, "#{count} #{action} requested")
     |> fetch_jobs()}
  end
  ...
  def handle_event("toggle_job", %{"id" => id}, socket) do
    ...
    {:noreply, assign(socket, selected_jobs: selected)}
  end

  @impl true
  def handle_event("toggle_all", _, socket) do
    ids = MapSet.new(socket.assigns.jobs, & &1.id)

    selected =
      if ids != MapSet.new() and MapSet.subset?(ids, socket.assigns.selected_jobs) do
        MapSet.difference(socket.assigns.selected_jobs, ids)
      else
        MapSet.union(socket.assigns.selected_jobs, ids)
      end
  ...
  def available_actions(state) do
    case state do
      ...
      state when state in ["complete", "completed"] -> ~w(retry delete)
      "available" -> ~w(run cancel delete)
      _ -> ~w(delete)
    end
  end
