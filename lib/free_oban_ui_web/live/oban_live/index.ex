defmodule FreeObanUiWeb.ObanLive.Index do
  ...
  def handle_event("toggle_all", _, socket) do
    ids = MapSet.new(Enum.map(socket.assigns.jobs, &to_string(&1.id)))

    selected =
      if MapSet.size(ids) > 0 and MapSet.equal?(ids, socket.assigns.selected_jobs) do
        MapSet.new()
      else
        ids
      end
  ...
  def from_now_short(later, now \\ DateTime.utc_now()) do
    diff = DateTime.diff(to_datetime(now), to_datetime(later))
