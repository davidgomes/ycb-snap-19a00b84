defmodule Paginator.Ecto.Query.FieldOrExpression do
  ...
  def field_or_expr_is_nil(%{column: {_name, handler}}) when is_function(handler, 0) do
  ...
  def field_or_expr_equal(%{column: {_name, handler}} = args) when is_function(handler, 0) do
  ...
  def field_or_expr_less(%{column: {_name, handler}} = args) when is_function(handler, 0) do
    dynamic(^handler.() < ^args.value)
  end
  ...
  def field_or_expr_greater(%{column: {_name, handler}} = args) when is_function(handler, 0) do
    dynamic(^handler.() > ^args.value)
  end
