defmodule Paginator.Ecto.Query.FieldOrExpression do
  ...
  def field_or_expr_equal(%{column: {_name, handler}, value: value})
      when is_function(handler, 0) do
  ...
  def field_or_expr_less(%{column: {_name, handler}, value: value})
      when is_function(handler, 0) do
    dynamic(^handler.() < ^value)
  end
  ...
  def field_or_expr_greater(%{column: {_name, handler}, value: value})
      when is_function(handler, 0) do
    dynamic(^handler.() > ^value)
  end
