defmodule Paginator.Ecto.Query.DynamicFilterBuilder do
  ...
  # PostgreSQL defaults: ASC is NULLS LAST and DESC is NULLS FIRST.
  @dispatch_table %{
  ...
  def build!(input) do
    case Map.fetch(@dispatch_table, input.sort_order) do
      ...
      :error ->
        available_sort_orders = Map.keys(@dispatch_table) |> Enum.sort() |> Enum.join(", ")

        raise(
          "Invalid sorting value :#{input.sort_order}, please use either #{available_sort_orders}"
        )
    end
  end
end
