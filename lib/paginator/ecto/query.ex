defmodule Paginator.Ecto.Query do
  ...
  defp build_where_expression(query, fields, values, cursor_direction) do
    fields
    |> Enum.reverse()
    |> Enum.reduce(true, fn {column, order}, next_filters ->
      {position, bound_column} = column_position(query, column)
