defmodule Paginator.Ecto.Query do
  ...
  defp cursor_key({name, expression}) when is_atom(name) and is_function(expression, 0),
    do: name

  defp cursor_key(column), do: column

  # Expressions carry their own bindings, so their position is never used
  defp column_position(_query, {name, expression})
       when is_atom(name) and is_function(expression, 0) do
    {0, expression.()}
  end
