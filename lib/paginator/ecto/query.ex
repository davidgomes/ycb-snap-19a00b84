defmodule Paginator.Ecto.Query do
  ...
  defp cursor_key({name, handler}) when is_function(handler, 0), do: name
  defp cursor_key(column), do: column

  # Expressions carry their own bindings, so no position lookup is needed
  defp column_position(_query, {_name, handler} = expression) when is_function(handler, 0),
    do: {0, expression}
