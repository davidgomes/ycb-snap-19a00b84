defmodule Paginator.Ecto.Query do
  ...
  defp cursor_key({name, handler}) when is_atom(name) and is_function(handler, 0), do: name
  defp cursor_key(column), do: column

  # Expressions resolve their own bindings, so the position is irrelevant
  defp column_position(_query, {_name, handler} = column) when is_function(handler, 0),
    do: {0, column}
