defmodule Guardian.Token.Jwt do
  ...
  * `secret` The secret key to use for signing. When omitted the configured
    `secret_key` is used; an explicit `nil` fails with `{:error, :secret_not_found}`
  ...
  * `ttl` - The time to live. See `Guardian.Token.ttl` type
  ...
  defmodule SecretFetcher.SecretFetcherDefaultImpl do
    ...
    # An explicit `secret: nil` must not fall back to `secret_key`: a per-request
    # lookup that found nothing would otherwise use the application wide secret.
  ...
  * `ttl` - The time to live. See `Guardian.Token.ttl` type
  ...
  Returns `{:error, :secret_not_found}` when no verifying secret could be
  found, and `{:error, :invalid_token}` for any other failure.
