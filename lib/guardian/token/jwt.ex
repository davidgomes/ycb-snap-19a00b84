defmodule Guardian.Token.Jwt do
  ...
  * `secret` The secret key to use for signing or verifying. When omitted the
    configured `secret_key` is used. An explicit `nil` (or a value resolving to
    `nil`) is not replaced by `secret_key` and fails with `{:error, :secret_not_found}`
  ...
  * `ttl` - The time to live. See `Guardian.Token.ttl` type
  ...
  defmodule SecretFetcher.SecretFetcherDefaultImpl do
    ...
    # Only an omitted `:secret` falls back to `secret_key`. A `:secret` that is
    # or resolves to `nil` is a failed lookup and must not sign or verify with
    # the application wide secret.
  ...
  A `secret` option that is (or resolves to) `nil` does not fall back to the
  configuration and returns `{:error, :secret_not_found}`.
  ...
  * `allowed_algos` - A list of allowable algos
  ...
  including when `secret` is given but is (or resolves to) `nil`.
  """
