if Code.ensure_loaded?(Plug) do
  defmodule Guardian.Plug.VerifyCookie do
    ...
    * `:secret` - The secret used to verify and sign the exchanged token. Any
      value resolvable by `Guardian.Config` is valid. A function of arity one is
      called with the connection so that the secret can be selected per request.
      See `Guardian.Plug.maybe_resolve_secret/2`.
    """
    ...
    def refresh_from_cookie(conn, opts) do
      with nil <- Guardian.Plug.current_token(conn, opts),
           ...
           exchange_opts <- Guardian.Plug.maybe_resolve_secret(conn, opts),
           {:ok, _old, {new_t, new_c}} <-
