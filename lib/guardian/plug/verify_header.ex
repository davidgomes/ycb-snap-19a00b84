if Code.ensure_loaded?(Plug) do
  defmodule Guardian.Plug.VerifyHeader do
    ...
    * `secret` - The secret used to verify the token instead of the configured one.
      It may also be a one argument function that is called with the connection
      and returns the secret for this request. A function returning `nil` rejects
      the token with `:secret_not_found`. See `Guardian.Plug.resolve_secret/2`.
    ...
    * `:halt` - Whether to halt the connection in case of error. Defaults to `true`
    * `:secret` - The secret used to verify the cookie and sign the exchanged token.
      Accepts a one argument function of the connection, as above.
