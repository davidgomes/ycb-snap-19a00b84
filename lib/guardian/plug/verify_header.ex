if Code.ensure_loaded?(Plug) do
  defmodule Guardian.Plug.VerifyHeader do
    ...
    * `secret` - The secret used to verify the token. Accepts any value supported by
      the token module, or a one-arity function that receives the connection and
      returns the secret.
    ...
    * `:halt` - Whether to halt the connection in case of error. Defaults to `true`
    ...
    def call(conn, opts) do
      with nil <- Guardian.Plug.current_token(conn, opts),
           ...
           verify_opts <- Guardian.Plug.put_secret_from_conn(opts, conn),
