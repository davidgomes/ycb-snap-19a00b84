defmodule ObanEvents do
  ...
  - Any option accepted by `Oban.Job.new/2` can be passed under `:oban` and is merged over the module defaults.
  ...
  defmacro __using__(opts) do
    ...
    # Global Oban job defaults, with any user-supplied options layered on top
    oban_defaults = Keyword.merge(@default_oban_opts, oban_opts)
    ...
    quote do
      ...
      @oban_defaults unquote(oban_defaults)
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Merge per-handler oban options (grouped under :oban key) with global defaults
            job_opts = Keyword.merge(@oban_defaults, Keyword.get(handler_opts, :oban, []))
