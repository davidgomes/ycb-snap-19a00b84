defmodule ObanEvents do
  ...
  3. An `ObanEvents.Event` is built with a unique `event_id`
  4. Oban jobs are created (one per handler)
  5. Jobs are persisted to the database within the transaction
  6. `DispatchWorker` processes each job asynchronously
  7. Each handler's `handle_event/2` callback is invoked with the `ObanEvents.Event`
  ...
  Accepts `:causation_id`, `:correlation_id`, and `:metadata` options.
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      - `opts`: Keyword list of event options:
        - `:causation_id` - `event_id` of the event that caused this one
        - `:correlation_id` - Correlation identifier (defaults to the new event's `event_id`)
        - `:metadata` - JSON-serializable map of additional metadata

      All handlers of a single emission receive the same `ObanEvents.Event`,
      including its unique `event_id`.
      ...
      ## Examples
          ...
          # With metadata and causation from a parent event
          #{inspect(__MODULE__)}.emit(
            :welcome_email_requested,
            %{user_id: user.id},
            ObanEvents.Event.caused_by(parent_event, metadata: %{source: "signup"})
          )
      ...
      def emit(event_name, data, opts \\ [])
        ...
        jobs =
          Enum.map(handlers, fn handler_module ->
            DispatchWorker.new(
              ObanEvents.Event.to_job_args(event, handler_module),
              queue: @oban_queue,
