defmodule ObanEvents do
  ...
  Any other `Oban.Job` option (`unique`, `schedule_in`, `scheduled_at`, `meta`, and so on)
  can be set the same way and is forwarded to `Oban.Job.new/2`.

  Per-handler options (override globals):
  - Handlers can be atoms (use defaults) or `{Handler, oban: [priority: 0, unique: [period: 60]]}`
  - The `:oban` keyword is merged over the module defaults, so unspecified keys keep the global value
  ...
  defmacro __using__(opts) do
    ...
    # The option is still AST here (`MyApp.Oban` is an alias tuple), so keep the
    # module form intact and only validate the keyword config before unquoting.
    {oban_module, oban_opts} =
      case Keyword.get(opts, :oban, Oban) do
        {module, config} when is_list(config) ->
          unless Keyword.keyword?(config) do
            raise ArgumentError, """
            Invalid :oban option: #{inspect({module, config})}

            Expected an Oban module, or {module, keyword} of Oban.Job options:

              use ObanEvents, oban: MyApp.Oban
              use ObanEvents, oban: {MyApp.Oban, queue: :events, max_attempts: 5, unique: [period: 60]}
            """
          end
    ...
    job_opts = Keyword.merge(@default_job_opts, oban_opts)
    ...
    quote do
      ...
      @oban_job_opts unquote(job_opts)
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Per-handler `:oban` options override module defaults. Every key is
            # forwarded, so options such as :unique and :schedule_in work too.
