defmodule ObanEvents do
  ...
  defmacro __using__(opts) do
    ...
    # Merge user-supplied options over the defaults. Any Oban.Job option is
    # allowed here, not just the ones with built-in defaults, so callers can
    # also set things like `unique` or `scheduled_at` globally.
    ...
    quote do
      ...
      # Global Oban job option defaults (can be overridden per-handler)
      @oban_opts unquote(global_oban_opts)
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Extract oban-specific options (grouped under :oban key) and merge
            # them over the global defaults, so any Oban.Job option can be
            # overridden per-handler, not just the ones with built-in defaults.
            oban_opts = Keyword.get(handler_opts, :oban, [])
            job_opts = Keyword.merge(@oban_opts, oban_opts)
