defmodule ObanEvents do
  ...
  Both the global and per-handler Oban options accept any `Oban.Job.new/2` option
  (e.g. `:scheduled_at`, `:schedule_in`, `:meta`, `:unique`). Per-handler options are
  merged over the global ones.
  ...
  @default_job_opts [queue: :oban_events, max_attempts: 3, priority: 2, tags: []]
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      # Global Oban job options (can be overridden per-handler)
      @oban_job_opts Keyword.merge(unquote(@default_job_opts), unquote(oban_opts))
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Per-handler Oban options (grouped under :oban key) override the global ones
            job_opts = Keyword.merge(@oban_job_opts, Keyword.get(handler_opts, :oban, []))
  ...
  # Runs after ObanEvents.Registry has validated the overall shape of @events
  defp validate_handler_opts!(env) do
    invalid_handlers =
      for {event, handlers} <- Module.get_attribute(env.module, :events),
          {_module, opts} = handler <- handlers,
          not valid_handler_opts?(opts),
          do: {event, handler}
    ...
    unless Enum.empty?(invalid_handlers) do
      raise CompileError,
        ...
        line: env.line,
        description:
          "handler options only support the :oban key with a keyword list of Oban.Job options " <>
            "(e.g. {MyHandler, oban: [priority: 0]}), found invalid handlers: #{inspect(invalid_handlers)}"
    end
  end

  defp valid_handler_opts?(opts) do
    Keyword.keyword?(opts) and Keyword.delete(opts, :oban) == [] and
      Keyword.keyword?(Keyword.get(opts, :oban, []))
  end
end
