defmodule ObanEvents do
  ...
  Per-handler options (override globals):
    ...
    - Any other [Oban job option](https://hexdocs.pm/oban/Oban.Job.html#new/2) under `:oban` is forwarded as-is (`unique`, `schedule_in`, `meta`, and so on)
  ...
  defmacro __using__(opts) do
    ...
    # Defaults for the subset we always set. Other Oban.Job options are
    # forwarded only when the caller supplies them.
    oban_defaults =
      [
        queue: :oban_events,
        max_attempts: 3,
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Per-handler :oban options override globals and may include any
            # Oban.Job.new/2 option (unique, schedule_in, meta, ...).
