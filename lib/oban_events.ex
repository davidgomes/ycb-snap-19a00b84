defmodule ObanEvents do
  ...
  receive an `ObanEvents.Event` struct holding the event data and metadata:
  ...
  Accepts the `:causation_id` and `:correlation_id` options, which are passed to
  handlers on the `ObanEvents.Event` struct.
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      - `opts`: Keyword list of event metadata (see Options)
      ...
      Every emit also generates an `event_id`, shared by all handler jobs, and a
      per-job `idempotency_key`. See `ObanEvents.Event` for details.
      ...
      ## Examples
          ...
          # From a handler, linking the new event to the one being handled
          #{inspect(__MODULE__)}.emit(:welcome_email_requested, event.data,
