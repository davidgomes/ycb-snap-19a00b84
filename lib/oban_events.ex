defmodule ObanEvents do
  ...
  Per-handler options (override globals):
    ...
    - Any other `Oban.Job` option (`unique`, `schedule_in`, `meta`, …) is forwarded as-is. Per-handler values replace the matching global option.
  ...
  defmacro __using__(opts) do
    ...
    # Defaults first so caller options (including unique, schedule_in, meta, …) win.
    oban_opts =
      Keyword.merge(
    ...
    quote do
      ...
      # Global Oban job options (per-handler :oban keyword replaces matching keys)
      @oban_job_opts unquote(oban_opts)
