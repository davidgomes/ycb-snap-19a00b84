defmodule ObanEvents do
  ...
  - Any option accepted by `Oban.Job.new/2` is forwarded (`unique`, `schedule_in`, `meta`, `replace`, and so on). Handler options override the module defaults.
  ...
  defmacro __using__(opts) do
    ...
    # Defaults for Oban.Job.new/2. Caller and per-handler options override these.
    oban_job_opts =
      Keyword.merge(
    ...
    quote do
      ...
      @oban_opts unquote(oban_job_opts)
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Per-handler Oban options override module defaults. Any
            # Oban.Job.new/2 option is forwarded (unique, schedule_in, meta, ...).
