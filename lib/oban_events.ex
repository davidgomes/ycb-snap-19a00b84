defmodule ObanEvents do
  ...
  Any other option accepted by `Oban.Job.new/2` (`meta`, `unique`, `schedule_in`,
  `scheduled_at`, and so on) can be set in the global `:oban` config and is used
  as the default for every handler.
  ...
  - Any `Oban.Job.new/2` option may be set under `:oban`. Per-handler values replace the global value for that key.
  ...
  defmacro __using__(opts) do
    ...
    # Global Oban job defaults. Caller options replace these keys and may add
    # any other option accepted by Oban.Job.new/2.
    oban_opts =
      Keyword.merge(
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Per-handler :oban options replace global defaults key by key.
            # Any option accepted by Oban.Job.new/2 is passed through.
            job_opts = Keyword.merge(@oban_opts, Keyword.get(handler_opts, :oban, []))
