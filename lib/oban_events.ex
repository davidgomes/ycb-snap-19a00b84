defmodule ObanEvents do
  ...
  - Handlers can be atoms (use defaults) or tuples with options: `{Handler, priority: 0, max_attempts: 10, queue: :critical, tags: ["urgent"]}` (or nested under `oban: [...]`)
  ...
  defmacro __using__(opts) do
    ...
    # - tuple with :opts keyword: oban: {MyApp.Oban, opts: [queue: :custom, ...]}
    {oban_module, oban_opts} =
      case Keyword.get(opts, :oban, Oban) do
        {module, config} when is_list(config) ->
          opts = Keyword.get(config, :opts, config)
          {module, opts}
    ...
    quote do
      ...
      @oban_job_opts unquote(default_job_opts)
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            ...
            # Extract handler oban options (support direct keyword list or nested under :oban key)
            oban_opts = Keyword.get(handler_opts, :oban, handler_opts)

            # Merge per-handler oban options with global defaults
            job_opts = Keyword.merge(@oban_job_opts, oban_opts)
