defmodule ObanEvents do
  ...
  Any `Oban.Job` option is accepted (for example `:queue`, `:max_attempts`, `:priority`,
  `:tags`, `:unique`, `:meta`), with these defaults:
  - `queue`: `:oban_events`
  - `max_attempts`: `3`
  - `priority`: `2` (0-3, lower is higher priority)
  - `tags`: `[]`

  Per-handler options (merged over the globals):
  ...
  @default_oban_opts [queue: :oban_events, max_attempts: 3, priority: 2, tags: []]

  @doc false
  @spec default_oban_opts() :: keyword()
  def default_oban_opts, do: @default_oban_opts
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @oban_job_opts Keyword.merge(ObanEvents.default_oban_opts(), unquote(oban_opts))
  ...
  @doc false
  @spec parse_handler(module() | {module(), keyword()}) :: {module(), keyword()}
  def parse_handler(module) when is_atom(module), do: {module, []}

  def parse_handler({module, opts}) when is_atom(module) and is_list(opts), do: {module, opts}

  @doc false
  @spec job_opts(keyword(), module(), keyword()) :: keyword()
  def job_opts(global_opts, handler_module, handler_opts) do
    case Keyword.get(handler_opts, :oban, []) do
      ...
      invalid ->
        ...
        Expected: a keyword list of Oban job options
        Got: #{inspect(invalid)}
        """
    end
  end
      ...
      def emit(event_name, data, opts \\ []) when is_atom(event_name) and is_map(data) do
        ...
        jobs =
          Enum.map(handlers, fn handler_spec ->
            {handler_module, handler_opts} = ObanEvents.parse_handler(handler_spec)
            ...
            DispatchWorker.new(
              ...
              ObanEvents.job_opts(@oban_job_opts, handler_module, handler_opts)
            )
          end)
