defmodule ErrorTracker do
  ...
  * A `{kind, exception}` tuple in which case the information is converted to
  ...
  - The `[:error_tracker, :occurrence, :new]` telemetry event includes
  `muted: true` in its metadata, so handlers can skip notifications for it
  - The error can still be manually resolved/unresolved
  """
