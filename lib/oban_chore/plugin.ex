defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:otp_app` - An atom or list of atoms representing the OTP application(s) to search for chores.
      Mutually exclusive with `:chores`.
    * `:chores` - An explicit list of chore modules. Use this instead of `:otp_app` when discovery
      should not scan an application (for example in tests). Mutually exclusive with `:otp_app`.
    * `:pubsub_server` - (Required) The name of your application's Phoenix PubSub server.

  Exactly one of `:otp_app` or `:chores` is required.
