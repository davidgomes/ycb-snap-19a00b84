defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:chores` - A list of chore modules to use instead of discovering them from `:otp_app`.
      Useful in tests. Cannot be combined with `:otp_app`.
  ...
  defp discover_chores(opts) do
    case Keyword.get(opts, :chores) do
      chores when is_list(chores) ->
        Enum.map(chores, fn module -> module.__chore_info__() end)
