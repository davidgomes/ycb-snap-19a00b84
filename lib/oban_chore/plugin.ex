defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:chores` - An explicit list of chore modules (modules that `use ObanChore.Worker`). When
      given, discovery is skipped and only these chores are shown, in the given order.
  ...
  Or, to register chores explicitly instead of discovering them:

  ```elixir
  {ObanChore.Plugin,
   chores: [MyApp.Chores.UserBackfill, MyApp.Chores.RefundOrder],
  ...
  defp validate_chores(opts) do
    case Keyword.get(opts, :chores) do
      ...
      chores when is_list(chores) ->
        case Enum.reject(chores, &chore?/1) do
          [] -> :ok
          invalid -> {:error, "all chores must use ObanChore.Worker, got: #{inspect(invalid)}"}
        end
  ...
  def handle_continue(:attach_telemetry, state) do
    ...
    # A handler left behind by a previous instance would keep broadcasting with its stale chores.
  ...
  defp discover_chores(opts) do
    modules =
      case Keyword.fetch(opts, :chores) do
        {:ok, chores} -> chores
        :error -> discover_chore_modules(opts)
      end

    Enum.map(modules, fn module -> module.__chore_info__() end)
  end

  # TODO: Improve the discovery
  defp discover_chore_modules(opts) do
    apps =
      case Keyword.get(opts, :otp_app) do
        nil ->
