defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:chores` - An explicit list of chore modules. Use this instead of `:otp_app` when chores
      should not be discovered by scanning applications, for example in tests.
    * `:pubsub_server` - (Required) The name of your application's Phoenix PubSub server.

  Set either `:otp_app` or `:chores`, but not both.
