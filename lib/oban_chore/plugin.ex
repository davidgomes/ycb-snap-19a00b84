defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:otp_app` - An atom or list of atoms representing the OTP application(s) to search for chores.
      Required unless `:chores` is set. Cannot be combined with `:chores`.
    * `:chores` - A list of chore modules to register explicitly, skipping application discovery.
      Required unless `:otp_app` is set. Cannot be combined with `:otp_app`.
      Each module must export `__chore_info__/0` (as `use ObanChore.Worker` does).
    * `:pubsub_server` - (Required) The name of your application's Phoenix PubSub server.

  ## Examples

  ```elixir
  # Discover chores from an OTP application:
  config :my_app, Oban,
    repo: MyApp.Repo,
    plugins: [
  ...
  # Or register an explicit list (useful in tests):
  config :my_app, Oban,
    repo: MyApp.Repo,
    plugins: [
