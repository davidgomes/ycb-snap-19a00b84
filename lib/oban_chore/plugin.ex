defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:otp_app` - An atom or list of atoms representing the OTP application(s) to search for chores.
      Required unless `:chores` is set. Mutually exclusive with `:chores`.
    * `:chores` - A list of chore modules to register explicitly. Required unless `:otp_app` is set.
      Mutually exclusive with `:otp_app`. Useful when chores live outside an OTP application
      module list, including in tests.
