defmodule ObanChore.Plugin do
  ...
  ## Options
    ...
    * `:otp_app` - An atom or list of atoms representing the OTP application(s) to search for chores.
      Required unless `:chores` is set.
    * `:chores` - A list of chore worker modules. When set, only these modules are registered
      instead of scanning `:otp_app`. Cannot be combined with `:otp_app`.
