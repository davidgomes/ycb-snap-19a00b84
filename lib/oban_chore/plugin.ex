defmodule ObanChore.Plugin do
    ...
    case Keyword.get(opts, :chores) do
      ...
      chores when is_list(chores) ->
        case Enum.reject(chores, &chore_module?/1) do
          [] -> :ok
          invalid -> {:error, "not ObanChore.Worker modules: #{inspect(invalid)}"}
        end
  ...
  def handle_continue(:attach_telemetry, state) do
    ...
    # A handler left behind by a previous plugin process would keep its stale config
