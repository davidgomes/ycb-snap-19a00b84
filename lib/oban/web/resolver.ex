defmodule Oban.Web.Resolver do
  ...
  * [Default Refresh](#c:resolve_refresh/1)—Set the default refresh interval for new sessions.
  ...
  Redact the signal payload for only the `ApprovalJob` worker:

      def format_signal(_signal, %Oban.Job{worker: "MyApp.ApprovalJob"}), do: "REDACTED"
  ...
  * `:update_crons`
  ...
  @doc """
  Decode an awaitable signal's payload from a compressed, base64 binary into proper terms.

  Signals are encoded the same way as recorded output, and decoding has the same `:safe` default
  as `decode_recorded/2`.

  ## Example

  Decode a signal binary:

      iex> Oban.Web.Resolver.decode_signal("g3QAAAABdwhkZWNpc2lvbm0AAAAIYXBwcm92ZWQ")
      %{decision: "approved"}
  """
