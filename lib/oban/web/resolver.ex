defmodule Oban.Web.Resolver do
  ...
  * [Default Refresh](#c:resolve_refresh/1)—Set the default refresh interval for new sessions.
  ...
  This callback is similar to `c:format_recorded/2`, but it receives the encoded payload of a
  signal delivered by `Oban.Pro.Worker.signal/2` along with the job to help augment the output.
  ...
  Redact the payload for a sensitive worker:

      def format_signal(_signal, %Oban.Job{worker: "MyApp.SecretApproval"}), do: "REDACTED"
  ...
  * `:update_crons`
  ...
  Signals are encoded the same way as recorded output, so decoding accepts the same options as
  `decode_recorded/2`, including the default `:safe` flag.
