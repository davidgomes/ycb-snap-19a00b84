defmodule Oban.Web.Jobs.DetailComponent do
  ...
  defp job_data_section(assigns) do
    ...
    <div class="px-3 py-6 border-t border-gray-200 dark:border-gray-700">
      ...
      <div id="job-data-content" class="mt-3">
        ...
        <.signal_output :if={signal_status(@job)} job={@job} resolver={@resolver} />
      </div>
    </div>
    """
  end
  ...
  defp signal_output(assigns) do
    assigns =
      case signal_status(assigns.job) do
        :received ->
          assign(assigns, status: :received, signal: format_signal(assigns.job, assigns.resolver))

        :awaiting ->
          assign(assigns, status: :awaiting, deadline: signal_deadline(assigns.job))
      end

    ~H"""
    <div id="job-signal" class="mt-4">
      <div class="relative bg-gray-50 dark:bg-gray-800 rounded-md p-4">
        <div class="flex justify-between items-start mb-2">
          <div class="flex items-center space-x-2">
            <h4 class="font-medium text-xs uppercase text-gray-500 dark:text-gray-400">
            ...
            <.pro_badge id="signal-pro-badge" tooltip="Signals from Oban.Pro.Worker" />
          </div>
          <button
            :if={@status == :received}
            type="button"
            ...
            data-title="Copy to clipboard"
            phx-hook="Tippy"
            phx-click={copy_to_clipboard(@signal)}
          >
            <Icons.icon name="icon-clipboard" class="w-4 h-4" />
          </button>
        </div>
        ...
        <%= if @status == :received do %>
          <pre class="font-mono text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap break-all">{@signal}</pre>
        <% else %>
          <div
            id="signal-deadline"
          ...
          >
            ...
            <%= if @deadline do %>
              <span>
                Deadline {Timing.datetime_to_words(@deadline)}
                <span class="text-gray-400 dark:text-gray-500">
                  ({@deadline |> DateTime.truncate(:second) |> to_string()})
                </span>
              </span>
            <% else %>
              <span>Waiting indefinitely</span>
            <% end %>
          </div>
        <% end %>
      </div>
  ...
  defp format_meta(%{meta: meta} = job, resolver) do
    ...
    meta = if signal_status(job) == :received, do: Map.delete(meta, "signal"), else: meta
  ...
  defp signal_status(%{meta: %{"signal" => signal}}) when is_binary(signal), do: :received

  defp signal_status(%{meta: %{"signal_deadline" => _}, state: state})
       when state not in ~w(cancelled completed discarded),
       do: :awaiting
  ...
  # Jobs that wait without a timeout persist "infinity" rather than a timestamp.
