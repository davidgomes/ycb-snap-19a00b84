defmodule FreeObanUi.Jobs do
  ...
  @doc """
  Fetches a single Oban job by its ID.
  """
  def get_job!(id) do
    Repo.get!(Job, id)
  end
  ...
  @doc """
  Counts jobs grouped by state. Returns a map with string state names as keys.
  """
  ...
  @doc """
  Returns a list of distinct queue names.
  """
  def list_queues, do: queues()
  ...
  defp base_query(filters) do
    Job
    |> filter_by_state(Map.get(filters, :state) || Map.get(filters, "state"))
    |> filter_by_queue(Map.get(filters, :queue) || Map.get(filters, "queue"))
  end

  defp filter_by_state(query, state) when state in [nil, "", "all"], do: query
  defp filter_by_state(query, state), do: where(query, [j], j.state == ^to_string(state))

  defp filter_by_queue(query, queue) when queue in [nil, "", "all"], do: query
  defp filter_by_queue(query, queue), do: where(query, [j], j.queue == ^to_string(queue))
  ...
  @doc """
  Deletes an Oban job from the database.
  """
  def delete_job(%Job{} = job) do
    Repo.delete(job)
  end

  def delete_job(id) when is_integer(id) or is_binary(id) do
    id
    |> get_job!()
    |> delete_job()
  end
