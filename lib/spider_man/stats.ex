defmodule SpiderMan.Stats do
  ...
  def get_stats(tid) do
    ...
    for component <- [:downloader, :spider, :item_processor] do
      {_, total, success, fail, duration} = Map.fetch!(stats, component)
      {component, %{total: total, success: success, fail: fail, tps: tps(success, duration)}}
    end
  end
  ...
  defp format_component_stats(
         {component, %{total: total, success: success, fail: fail, tps: tps}}
       ) do
