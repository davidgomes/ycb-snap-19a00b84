defmodule BroadwayKafka.Producer do
  ...
  ## Options
    ...
    * `:shared_client` - Optional. A boolean that defines how many clients will be started.
      ...
      will have its own client. Default is `false`.
  ...
  def init(opts) do
    ...
    client_id = config[:shared_client_id] || :"#{prefix}#{Module.concat([producer_name, Client])}"
  ...
  def prepare_for_start(_module, opts) do
    ...
    case client.init(Keyword.put(producer_opts, :broadway, opts)) do
      ...
      {:ok, extra_child_specs, config} ->
        new_producer_opts = Keyword.put(producer_opts, :initialized_client_config, config)
