defmodule BroadwayKafka.BrodClient do
  ...
  # The shared client is started by Broadway's supervision tree
  # (see shared_client_child_spec/1), so producers must not start it.
  ...
  def shared_client_child_spec(config) do
    %{shared_client_id: client_id, hosts: hosts, client_config: client_config} = config

    [
      %{
        id: client_id,
        start: {:brod, :start_link_client, [hosts, client_id, client_config]}
      }
    ]
  end
