defmodule Warehouse.Server do
  ...
  def list_component_availability(%{components: components}, stream) do
    ...
    Repo.transaction(
      fn ->
        query
        |> Repo.stream()
        |> Stream.map(&component_availability/1)
  ...
  defp component_availability(%Schemas.Component{} = component) do
    component_id = to_string(component.id)
    picking_options = Components.picking_options(component)
    available_quantity = Components.available_quantity(picking_options)

    Logger.info("Component #{component_id} has #{available_quantity} available")
