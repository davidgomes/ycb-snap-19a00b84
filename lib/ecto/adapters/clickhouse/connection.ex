defmodule Ecto.Adapters.ClickHouse.Connection do
  ...
  def quote_name(name) when is_atom(name), do: name |> Atom.to_string() |> quote_with(?")
  ...
  defp quote_with(value, quoter) do
    # Backslashes are escaped first so that the delimiter escapes added next stay intact.
    escaped =
      value
      |> IO.iodata_to_binary()
