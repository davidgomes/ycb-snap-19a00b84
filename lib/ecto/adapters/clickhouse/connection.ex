defmodule Ecto.Adapters.ClickHouse.Connection do
  ...
  def quote_name(names) when is_list(names) do
    ...
    |> intersperse_map(?., &to_string/1)
    |> quote_with(?")
  end

  def quote_name(name) when is_atom(name) do
    name |> Atom.to_string() |> quote_with(?")
  end

  def quote_name(name), do: quote_with(name, ?")
  ...
  # ClickHouse reads a doubled delimiter as a literal one inside both
  # quoted identifiers and string literals.
  defp quote_with(value, delimiter) do
    escaped =
      value
      |> IO.iodata_to_binary()
      |> :binary.replace("\\", "\\\\", [:global])
      |> :binary.replace(<<delimiter>>, <<delimiter, delimiter>>, [:global])
