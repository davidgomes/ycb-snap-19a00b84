defmodule Ecto.Adapters.ClickHouse.Connection do
  ...
  defp expr({:values, _, [types, idx, num_rows]}, _sources, params, query) do
    ...
    ["VALUES(", quote_string(IO.iodata_to_binary(structure)), ?,, rows, ?)]
  end
  ...
  def quote_name(name) when is_binary(name), do: quote_with(name, ?")

  @doc false
  def quote_string(value) when is_binary(value), do: quote_with(value, ?')
