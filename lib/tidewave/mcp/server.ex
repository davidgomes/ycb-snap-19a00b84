defmodule Tidewave.MCP.Server do
  ...
  @doc """
  Processes a decoded JSON-RPC MCP message and returns the result to be
  ...
  @doc """
  Validates that a decoded message conforms to the JSON-RPC 2.0 envelope
  expected by MCP, independently of the transport it arrived over.
  """
  def validate_jsonrpc_message(%{"jsonrpc" => "2.0"} = message) do
    cond do
  ...
  def validate_jsonrpc_message(_), do: {:error, :invalid_jsonrpc}
end
