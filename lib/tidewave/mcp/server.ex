defmodule Tidewave.MCP.Server do
  ...
  Returns `{:ok, nil}` for notifications, `{:ok, response}` on success,
  or `{:error, response}` when the response is a JSON-RPC error.
  An unparseable message returns `{:ok, response}` with a JSON-RPC error,
  as it is a valid reply at the transport level.
  """
  def handle_jsonrpc_message(params, assigns, include_browser_tools?) do
    case validate_jsonrpc_message(params) do
      {:ok, message} ->
        handle_message(message, assigns, include_browser_tools?)

      {:error, :invalid_jsonrpc} ->
        Logger.warning("Invalid JSON-RPC message format")

        {:ok,
         %{jsonrpc: "2.0", id: nil, error: %{code: -32600, message: "Could not parse message"}}}
    end
  end
end
