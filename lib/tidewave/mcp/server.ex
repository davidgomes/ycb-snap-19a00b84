defmodule Tidewave.MCP.Server do
  ...
  # Handles a decoded JSON-RPC message, returning either
  #
  #   * `{:ok, nil}` if there is nothing to reply (e.g. notifications)
  #   * `{:ok, response}` with a JSON-RPC result response
  #   * `{:error, response}` with a JSON-RPC error response
  #   * `{:error, :invalid_jsonrpc}` if the message is not valid JSON-RPC
  #
  @doc false
  def handle_message(message, assigns, include_browser_tools?) do
    Logger.debug("Received message: #{inspect(message, pretty: true)}")
