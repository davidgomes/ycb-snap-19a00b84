defmodule ObanDoctor.Check.Worker.UniquenessMissingStatesTest do
  ...
  describe "run/1" do
    ...
    test "returns warning when worker uses :scheduled state group which is missing :executing" do
      workers = [
        %{
      ...
      assert length(issues) == 1
      [issue] = issues
      assert issue.meta.missing_states == [:executing]
    end
    ...
    test "does not flag workers using :final state group (handled by another check)" do
      workers = [
        %{
          module: MyApp.Workers.FinalStatesWorker,
          file: "lib/my_app/workers/final_states_worker.ex",
          line: 1,
          queue: :default,
