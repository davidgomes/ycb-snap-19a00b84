defmodule ObanDoctor.Check.Worker.UniquenessMissingStatesTest do
  ...
  describe "run/1" do
    ...
    test "returns no issues for named groups covering all recommended states" do
      workers =
        for group <- [:incomplete, :successful] do
          %{
            module: Module.concat(MyApp.Workers, "#{group}_worker"),
            file: "lib/my_app/workers/#{group}_worker.ex",
            line: 1,
            queue: :default,
    ...
    test "detects workers using the :scheduled named group" do
      workers = [
        %{
