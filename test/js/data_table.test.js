...
describe("PetalDataTable", () => {
  ...
  it("filter apply replaces this field's committed entry", () => {
    ...
    // the panel is in the page and LiveView owns its display - a hook
    // writing style here would be replayed open by the patch above
  ...
  it("event mode: submit passes straight through - no interception, no navigation", () => {
    ...
    expect(patched).toEqual([]);
    // the form's own phx-submit pushes AND hides the panel
