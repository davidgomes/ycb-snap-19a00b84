defmodule Canary.HooksTest do
  ...
  describe "handle_hook/2" do
    ...
    test "uses the pluralized resource name for the :index action like plugs" do
      uri = "http://localhost/post"
      metadata = %{hook: :authorize_resource, stage: :handle_params, opts: [model: Post]}

      socket =
        build_socket(:index)
        |> put_assigns(%{posts: %Post{id: 1}, current_user: %User{id: 1}})

      assert {:cont, socket} = Canary.Hooks.handle_hook(metadata, [%{}, uri, socket])
      assert socket.assigns.authorized == true
      assert socket.assigns.canary_action == :index
    end
  ...
  describe "integration for :load_resource" do
    ...
    test "it halt the socket when the resource is required", %{conn: conn} do
      assert {:error, {:redirect, %{to: "/"}}} = live(conn, "/post/13")
