defmodule Flop.Adapter.Ecto.DialectTest do
  ...
  describe "the cursor predicate" do
    ...
    test "keeps the following order fields when the cursor value is nil" do
      clause =
        cursor_clause(PostgresRepo, [:age, :name], [:asc, :asc], %{
          age: nil,
          name: "Ada"
        })
    ...
    test "includes null rows after a value when they sort last" do
      clause =
        cursor_clause(PostgresRepo, [:age, :name], [:asc_nulls_last, :asc], %{
          age: 7,
          name: "Ada"
        })

      assert clause =~ "is_nil(p0.age)"
      assert clause =~ "p0.age > "
      assert clause =~ "p0.name > "
    end
  end
  ...
  defp cursor_clause(repo, direction, cursor_value) do
    cursor_clause(repo, [:name], [direction], %{name: cursor_value})
  end

  defp cursor_clause(repo, fields, directions, cursor_value) do
    flop = %Flop{
      first: 2,
      after: Flop.Cursor.encode(cursor_value),
      order_by: fields,
