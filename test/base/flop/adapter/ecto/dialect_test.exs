defmodule Flop.Adapter.Ecto.DialectTest do
  ...
  describe "the cursor predicate" do
    ...
    test "applies the null predicate to a join field" do
      import Ecto.Query

      query =
        from p in MyApp.Pet,
          left_join: o in assoc(p, :owner),
          as: :owner

      flop = %Flop{
        first: 2,
        after: Flop.Cursor.encode(%{owner_age: 4}),
        order_by: [:owner_age],
      ...
      assert clause =~ "o1.age > "
      assert clause =~ "is_nil(o1.age)"
    end

    test "keeps the null rows when a tiebreaker follows a null cursor value" do
      flop = %Flop{
        first: 2,
        after: Flop.Cursor.encode(%{age: nil, name: "m"}),
        order_by: [:age, :name],
      ...
      assert clause =~ "is_nil(p0.age)"
      assert clause =~ "p0.name > "
    end

    test "builds a query when every cursor value is nil and nulls sort last" do
      flop = %Flop{
        first: 2,
        after: Flop.Cursor.encode(%{age: nil, name: nil}),
        order_by: [:age, :name],
        order_directions: [:asc_nulls_last, :asc_nulls_last]
      }

      assert %Ecto.Query{} =
               Flop.query(MyApp.Pet, flop, for: MyApp.Pet, repo: PostgresRepo)
    end
  end
