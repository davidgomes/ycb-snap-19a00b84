defmodule Flop.SchemaTest do
  ...
  test "raises if a filterable custom field has no filter or field_dynamic" do
    error =
      assert_raise ArgumentError, fn ->
        defmodule Sage do
          @derive {
    ...
    assert error.message =~
             "custom field without field_dynamic or filter function" <>
  ...
  test "allows a filterable custom field with only field_dynamic" do
    ...
    assert %Flop.FieldInfo{
             extra: %{
               type: :custom,
               filter: nil,
               field_dynamic: {SageWithDynamic, :some_function, []}
             }
           } = Schema.field_info(struct(SageWithDynamic), :inserted_at)
  end
