defmodule Flop.SchemaTest do
  ...
  test "raises error if a filterable custom field has no filter or field_dynamic" do
    error =
      assert_raise ArgumentError, fn ->
        defmodule Sage do
          @derive {
  ...
  test "allows a filterable custom field that only configures field_dynamic" do
    ...
    assert %Flop.FieldInfo{
             extra: %{
               type: :custom,
               filter: nil,
               field_dynamic: {Rosemary, :some_function, []}
             }
