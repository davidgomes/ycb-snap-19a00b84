defmodule Sqlite.Ecto2.Test do
  ...
  test "update all with returning" do
    ...
    query = from(m in Schema, update: [set: [x: 0]]) |> select([m], {m.id, m.y}) |> normalize(:update_all)
    assert update_all(query) ==
           ~s{UPDATE "schema" SET "x" = 0 ;--RETURNING ON UPDATE "schema","id","y"}
  end
  ...
  test "create table with bigserial primary key and references" do
    create = {:create, table(:posts),
               [{:add, :id, :bigserial, [primary_key: true]},
                {:add, :category_0, references(:categories, type: :bigserial), []},
                {:add, :category_1, references(:categories, type: :serial), []}]}

    assert execute_ddl(create) == ["""
    CREATE TABLE "posts" ("id" INTEGER PRIMARY KEY AUTOINCREMENT,
    "category_0" INTEGER CONSTRAINT "posts_category_0_fkey" REFERENCES "categories"("id"),
    "category_1" INTEGER CONSTRAINT "posts_category_1_fkey" REFERENCES "categories"("id"))
    """ |> remove_newlines]
  end
