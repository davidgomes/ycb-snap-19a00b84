defmodule ObanChore.QueriesTest do
  ...
  test "list_history_jobs/2 generates the correct Ecto query", %{oban_name: oban_name} do
    ObanChore.list_history_jobs(SomeWorker, oban_name)

    assert_receive {:repo_all, query}
    assert query.from.source == {"oban_jobs", Oban.Job}

    query_str = inspect(query)
    assert query_str =~ "j0.state in [\"completed\", \"discarded\", \"cancelled\"]"
    assert query_str =~ "j0.worker == ^\"SomeWorker\""
    assert query_str =~ "order_by: [desc: fragment(\"coalesce(?, ?, ?)\""
