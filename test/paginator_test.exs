defmodule PaginatorTest do
  ...
  for order <- @available_sorting_orders do
    test "raises an error if nulls are used in the last term - order_by charged_at #{order}" do
    ...
    test "paginates correctly when pages contain nulls - order by charged_at #{field0_order}, id #{
      ...
      for k <- 1..50 do
        charged_at =
          case rem(k, 4) do
            0 -> nil
            1 -> NaiveDateTime.add(now, k)
            _ -> NaiveDateTime.add(now, k - 1)
          end
