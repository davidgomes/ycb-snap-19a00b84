defmodule Sentry.TelemetryProcessorIntegrationTest do
  ...
  describe "buffer overflow client reports" do
    ...
    test "sends cache_overflow client report when log buffer overflows", ctx do
      ...
      cache_overflow =
        Enum.filter(client_report["discarded_events"], &(&1["reason"] == "cache_overflow"))

      assert %{"quantity" => 1} = Enum.find(cache_overflow, &(&1["category"] == "log_item"))
      assert %{"quantity" => bytes} = Enum.find(cache_overflow, &(&1["category"] == "log_byte"))
      assert bytes > 0
