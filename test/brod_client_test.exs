defmodule BroadwayKafka.BrodClientTest do
  ...
  describe "validate init options" do
    ...
    test ":shared_client is an optional boolean. Default is false" do
      ...
      opts = Keyword.put(@opts, :shared_client, false)
      assert {:ok, [], %{shared_client: false, shared_client_id: nil}} = BrodClient.init(opts)
    end
    ...
    test "returns the shared client child spec when :shared_client is true" do
      ...
      assert {:ok, [child_spec], %{shared_client: true, shared_client_id: client_id}} =
               BrodClient.init(opts)

      assert client_id == MyBroadway.SharedClient

      assert child_spec == %{
               id: client_id,
               start: {:brod, :start_link_client, [[host: 9092], client_id, []]}
             }
    end

    test "shared client id respects :client_id_prefix" do
      opts =
        @opts
        |> Keyword.put(:shared_client, true)
        |> Keyword.put(:broadway, name: MyBroadway)
      ...
      assert {:ok, [%{id: client_id}], %{shared_client_id: client_id}} = BrodClient.init(opts)
      assert client_id == :"my_prefix - Elixir.MyBroadway.SharedClient"
    end
  end
