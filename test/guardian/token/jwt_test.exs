defmodule Guardian.Token.JwtTest do
  ...
  describe "with a secret fetcher that inherits the defaults" do
    ...
    test "fails closed on a nil :secret rather than falling back", ctx do
      assert {:error, :secret_not_found} = Guardian.decode_and_verify(InheritedImpl, ctx.token, %{}, secret: nil)

      assert {:error, :secret_not_found} = Guardian.encode_and_sign(InheritedImpl, @resource, %{}, secret: nil)
    end

    test "honours an explicit :secret", ctx do
      assert {:error, :invalid_token} = Guardian.decode_and_verify(InheritedImpl, ctx.token, %{}, secret: "another-key")
    end
  end
