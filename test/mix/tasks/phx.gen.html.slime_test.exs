...
defmodule Mix.Tasks.Phx.Gen.Html.SlimeTest do
  ...
  test "generates html resource and handles existing contexts" do
    in_tmp "generates html resource and handles existing contexts", fn ->
      ...
      assert_file "lib/phoenix_slime_web/templates/comment/index.html.slime", fn file ->
        assert file =~ "= for comment <- @comments do"
      end
  ...
  test "with --web namespace generates namespaced web modules and directories" do
    in_tmp "with --web namespace generates namespaced web modules and directories", fn ->
  ...
  test "with --no-context skips context and schema file generation" do
    in_tmp "with --no-context skips context and schema file generation", fn ->
  ...
  test "with --no-schema skips schema file generation" do
    in_tmp "with --no-schema skips schema file generation", fn ->
