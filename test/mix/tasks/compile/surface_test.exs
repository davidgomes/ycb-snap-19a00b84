defmodule Mix.Tasks.Compile.SurfaceTest do
  ...
  test "use manifest to avoid re-compilation and keep track of diagnostics" do
    # Running the first time compiles it normally, returning diagnostics
    assert {:ok, [%Diagnostic{} | _]} = run(["--return-errors", "--force"])

    # Running again compiles nothing and returns no warnings as there's no change
    assert {:noop, []} = run(["--return-errors"])

    # Running again using --all-warnings compiles nothing but returns existing warnings
