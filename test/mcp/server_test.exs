defmodule Tidewave.MCP.ServerTest do
  ...
  describe "handle_message/3" do
    test "handles initialization message" do
      ...
      assert {:ok, response} = Tidewave.MCP.Server.handle_message(message, assigns(), true)
    ...
    test "handles initialized notification" do
      ...
      assert {:ok, nil} = Tidewave.MCP.Server.handle_message(message, assigns(), true)
    end
    ...
    test "does not include browser tools when disabled" do
      ...
      assert {:ok, response} = Tidewave.MCP.Server.handle_message(message, assigns(), false)
    ...
    test "does not dispatch browser tools when disabled" do
      ...
      assert {:error, response} = Tidewave.MCP.Server.handle_message(message, assigns(), false)
    ...
    test "returns invalid arguments for malformed browser tool arguments" do
      ...
      assert {:error, response} = Tidewave.MCP.Server.handle_message(message, assigns(), true)
