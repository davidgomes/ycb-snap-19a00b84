defmodule Tidewave.MCP.ServerTest do
  ...
  describe "handle_message/3" do
    test "handles initialization message" do
      ...
      assert {:ok, response} = handle_message(message)
    ...
    test "handles initialized notification" do
      ...
      assert {:ok, nil} = handle_message(message)
    end
    ...
    test "does not include browser tools when disabled" do
      ...
      assert {:ok, response} = handle_message(message, false)
    ...
    test "does not dispatch browser tools when disabled" do
      ...
      assert {:error, response} = handle_message(message, false)
    ...
    test "returns invalid arguments for malformed browser tool arguments" do
      ...
      assert {:error, response} = handle_message(message, true)
    ...
    test "returns error for invalid JSON-RPC message" do
      ...
      log =
        capture_log([level: :warning], fn ->
          assert {:error, :invalid_jsonrpc} = handle_message(message)
        end)
  ...
  defp handle_message(message, include_browser_tools? \\ true) do
    Tidewave.MCP.Server.handle_message(message, @config, include_browser_tools?)
  end
end
