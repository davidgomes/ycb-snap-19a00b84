defmodule Tidewave.MCP.ServerTest do
  ...
  describe "handle_message/3" do
    test "handles initialization message" do
      ...
      assert {:ok, response} = Server.handle_message(message, @config, true)
      response_body = roundtrip(response)
    ...
    test "handles initialized notification" do
      ...
      assert Server.handle_message(message, @config, true) == {:ok, nil}
    end
    ...
    test "does not include browser tools when disabled" do
      ...
      assert {:ok, response} = Server.handle_message(message, @config, false)
      response_body = roundtrip(response)
    ...
    test "does not dispatch browser tools when disabled" do
      ...
      assert {:error, response} = Server.handle_message(message, @config, false)
      response_body = roundtrip(response)
    ...
    test "returns invalid arguments for malformed browser tool arguments" do
      ...
      assert {:error, response} = Server.handle_message(message, @config, true)
      response_body = roundtrip(response)
  ...
  defp roundtrip(response), do: response |> Jason.encode!() |> Jason.decode!()
end
