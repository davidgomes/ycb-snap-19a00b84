defmodule Phoenix.LiveView.UploadChannelTest do
  ...
  def cancel_and_reallow(lv, entry_ref) do
    UploadLive.run(lv, fn socket ->
      {:reply, :ok, Phoenix.LiveView.cancel_upload(socket, :avatar, entry_ref)}
    end)
  ...
  for context <- [:lv, :nested_lv, :component] do
    ...
    describe "#{@context} with valid token" do
      ...
      test "writer write_chunk/2 error self-closes the upload channel without bringing down the LiveView",
        ...
        assert_receive {:writer_failure_progress, %LiveView.UploadEntry{ref: ^ref},
                        [writer_failure: :custom_error]}

        refute_receive {:writer_failure_progress, _, _}

        # the failed entry can no longer be uploaded to, even with its still valid token
        {:ok, socket} = Phoenix.ChannelTest.connect(Phoenix.LiveView.Socket, %{})
      ...
      test "successful sibling entries can be consumed individually after a writer failure",
           %{lv: lv} do
        Process.register(self(), :test_writer)
        ...
        assert {[], [%LiveView.UploadEntry{client_name: "bad.jpeg", ref: bad_ref}]} =
                 get_uploaded_entries(lv, :avatar)
