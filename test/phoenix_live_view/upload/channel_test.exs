  ...
  for context <- [:lv, :nested_lv, :component] do
    ...
    describe "#{@context} with valid token" do
      ...
      test "writer write_chunk/2 error self-closes the upload channel without bringing down the LiveView",
        ...
        # the failed entry is retained with the writer failure until it is cancelled
        assert {[],
                [
        ...
        # the progress callback is invoked once with the failed entry
        ...
        # the retained entry may not be uploaded to again with its still valid token
        {:ok, socket} = Phoenix.ChannelTest.connect(Phoenix.LiveView.Socket, %{})
      ...
      test "writer init/1 error fails the entry without bringing down the LiveView", %{lv: lv} do
        ...
        # the entry is retained with the writer failure, just like a write_chunk/2 error
