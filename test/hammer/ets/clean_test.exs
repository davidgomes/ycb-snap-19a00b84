defmodule Hammer.ETS.CleanTest do
  ...
  defmodule RateLimitBeforeCleanExits do
    use Hammer, backend: :ets
  end
  ...
  describe "before_clean callback" do
    setup do
      ...
      %{
        callback: fn algorithm, entries -> send(test_pid, {:before_clean, algorithm, entries}) end
      }
    end

    test "receives correct algorithm atom and entries for fix_window", %{callback: callback} do
      ...
      assert expired_at <= System.system_time(:millisecond)
    ...
    test "receives correct algorithm atom and entries for sliding_window", %{callback: callback} do
      start_supervised!(
    ...
    test "receives correct algorithm atom and entries for token_bucket", %{callback: callback} do
      start_supervised!(
      ...
      assert [%{key: "user_1", value: 9, expired_at: expired_at}] = entries
      assert is_integer(expired_at)
    ...
    test "receives correct algorithm atom and entries for leaky_bucket", %{callback: callback} do
      start_supervised!(
    ...
    test "entries are deleted even when callback exits" do
      log =
        capture_log(fn ->
          start_supervised!(
            {RateLimitBeforeCleanExits,
             clean_period: 100, before_clean: fn _algorithm, _entries -> exit(:boom) end}
          )

          assert {:allow, 1} = RateLimitBeforeCleanExits.hit("user_1", 100, 10)

          eventually(fn ->
            :ets.tab2list(RateLimitBeforeCleanExits) == []
          end)
        end)
    ...
    test "MFA tuple form works" do
      ...
      start_supervised!(
        {RateLimitBeforeCleanMFA,
         clean_period: 100, before_clean: {Hammer.CallbackHandler, :handle, [test_pid]}}
      )
