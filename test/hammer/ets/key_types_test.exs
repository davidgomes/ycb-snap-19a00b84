defmodule Hammer.ETS.KeyTypesTest do
  ...
  test "sliding window accepts any term as key" do
    start_supervised!(RateLimitSlidingWindow)
    scale = :timer.minutes(10)
    limit = 2

    for key <- @keys do
      assert RateLimitSlidingWindow.get(key, scale) == 0
      assert {:allow, 1} = RateLimitSlidingWindow.hit(key, scale, limit)
      assert {:allow, 2} = RateLimitSlidingWindow.hit(key, scale, limit)
      assert RateLimitSlidingWindow.get(key, scale) == 2
      assert {:deny, _retry_after} = RateLimitSlidingWindow.hit(key, scale, limit)
    end
  end
