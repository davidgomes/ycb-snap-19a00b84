defmodule Flop.Adapters.Ecto.FlopTest do
  ...
  describe "ordering" do
    ...
    test "prefers a custom filter function over field_dynamic" do
      insert_custom_field_pets([20])

      assert Flop.all(
               CustomFieldPet,
               %Flop{
                 filters: [%Filter{field: :blocked_score, op: :==, value: 40}]
               },
               for: CustomFieldPet
             ) == []
    end

    test "casts filter values for a field_dynamic custom field" do
      assert {:ok, %Flop{filters: [%Filter{value: 40}]}} =
               Flop.validate(
