defmodule Flop.Adapters.Ecto.FlopTest do
  ...
  describe "ordering" do
    ...
    test "filters by a custom array field with contains" do
      Repo.insert!(%CustomFieldPet{name: "set", tags: ["a", "b"]})
      Repo.insert!(%CustomFieldPet{name: "other", tags: ["c"]})
      ...
      for {op, expected} <- [
        ...
        result =
          Flop.all(
            CustomFieldPet,
            %Flop{
              filters: [%Filter{field: :tag_list, op: op, value: "a"}]
            },
            for: CustomFieldPet
          )
