defmodule Surface.ComponentStyleTest do
  ...
  test "inject caller's scope attribute on live components implementing update/2, i.e. @__caller_scope_id__ is still there" do
    ...
    # We need to use `attr="attr"` (or `attr=""` in LiveView >= 1.1) instead of just `attr` here
    # because it seems live_isolated/2 renders attributes differently. Maybe because it relies
    # on `<.dynamic_tag/>`?
