defmodule TidewaveTest do
  ...
  describe "/mcp" do
    ...
    test "200 with JSON-RPC response for requests" do
      conn = mcp_request(%{"jsonrpc" => "2.0", "method" => "tools/list", "id" => "1"})
    ...
    test "202 for notifications" do
      conn = mcp_request(%{"jsonrpc" => "2.0", "method" => "notifications/initialized"})

      assert conn.status == 202
      assert Jason.decode!(conn.resp_body) == %{"status" => "ok"}
    end

    test "400 with JSON-RPC error for failed requests" do
      conn = mcp_request(%{"jsonrpc" => "2.0", "method" => "unknown", "id" => "1"})
  ...
  defp mcp_request(message, path \\ "/tidewave/mcp") do
