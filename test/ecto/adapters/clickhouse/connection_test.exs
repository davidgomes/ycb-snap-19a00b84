defmodule Ecto.Adapters.ClickHouse.ConnectionTest do
  ...
  test "quoted identifier and string escape match the byte-wise oracle for every byte pair" do
    ...
    for {quote_fun, quoter} <- [
          {&Connection.quote_name/1, ?"},
          {&Connection.quote_string/1, ?'}
        ] do
      expected =
  ...
  test "column default escapes single quotes and backslashes" do
    create =
      {:create, table(:posts, engine: "MergeTree"),
       [
         {:add, :name, :string, [default: "it's \\ fine"]}
       ]}

    assert execute_ddl(create) == [
             ~S{CREATE TABLE "posts" ("name" String DEFAULT 'it\'s \\ fine') ENGINE=MergeTree}
           ]
  end
