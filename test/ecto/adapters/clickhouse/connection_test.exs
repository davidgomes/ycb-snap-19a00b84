defmodule Ecto.Adapters.ClickHouse.ConnectionTest do
  ...
  test "quoted identifier escape" do
    ...
    assert Connection.quote_name(~s{has`tick'quote}) |> IO.iodata_to_binary() ==
             ~s{"has`tick'quote"}

    assert Connection.quote_name(:atom_name) |> IO.iodata_to_binary() == ~s{"atom_name"}
    assert Connection.quote_name(nil) == []
  ...
  test "quoted string escape" do
    assert Connection.quote_string(~s{it's}) |> IO.iodata_to_binary() == ~s{'it''s'}
    assert Connection.quote_string(~s{a\\b}) |> IO.iodata_to_binary() == ~s{'a\\\\b'}

    assert Connection.quote_string(~s{"double"`tick`}) |> IO.iodata_to_binary() ==
             ~s{'"double"`tick`'}

    assert Connection.quote_string(["io", ?', "data"]) |> IO.iodata_to_binary() ==
             ~s{'io''data'}
  end

  test "quoted identifier and string escape match the byte-wise oracle for every byte pair" do
    value = for left <- 0..255, right <- 0..255, into: <<>>, do: <<left, right>>

    for {quoter, quote_fun} <- [{?", &Connection.quote_name/1}, {?', &Connection.quote_string/1}] do
      expected =
      ...
      assert quote_fun.(value) |> IO.iodata_to_binary() ==
  ...
  test "json_extract_path" do
    ...
    query = Schema |> select([s], json_extract_path(s.meta, ["FROM", "select"]))
    assert all(query) == ~s{SELECT s0."meta"."FROM"."select" FROM "schema" AS s0}
    ...
    query = Schema |> select([s], json_extract_path(s.meta, ["a", 0, "b"]))
    assert all(query) == ~s{SELECT s0."meta"."a"[0]."b" FROM "schema" AS s0}
  ...
  test "column default and comment escape quotes and backslashes" do
    create =
      {:create, table(:posts, engine: "MergeTree"),
       [
         {:add, :name, :string, [default: ~S|it's \ "here"|, comment: ~S|a\'b|]}
       ]}

    assert execute_ddl(create) == [
             ~S|CREATE TABLE "posts" ("name" String DEFAULT 'it''s \\ "here"' COMMENT 'a\\''b') ENGINE=MergeTree|
           ]

    alter =
      {:alter, table(:posts), [{:modify, :name, :string, [default: ~S|x'y|, comment: ~S|c\d|]}]}

    assert execute_ddl(alter) == [
             ~S|ALTER TABLE "posts" MODIFY COLUMN "name" String DEFAULT 'x''y' COMMENT 'c\\d'|
           ]
  end
