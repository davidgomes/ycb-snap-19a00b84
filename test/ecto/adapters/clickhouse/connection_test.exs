defmodule Ecto.Adapters.ClickHouse.ConnectionTest do
  ...
  test "quoted values match the byte-wise oracle for every byte pair" do
    ...
    for {quoter, quote_fun} <- [{?', &Connection.quote_string/1}, {?", &Connection.quote_name/1}] do
      expected =
      ...
      assert quote_fun.(value) |> IO.iodata_to_binary() == <<quoter, expected::binary, quoter>>
    end
  end
  ...
  test "string defaults and comments escape quotes and backslashes" do
    create =
      {:create, table(:posts, engine: "MergeTree"),
       [
         {:add, :name, :string, [default: ~S|it\'s|, comment: ~S|a\'b|]}
       ]}

    assert execute_ddl(create) == [
             ~S|CREATE TABLE "posts" ("name" String DEFAULT 'it\\''s' COMMENT 'a\\''b') ENGINE=MergeTree|
           ]
  end
