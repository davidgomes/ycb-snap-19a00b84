defmodule ObanEventsTest do
  ...
  # Test module passing through extra Oban job options
  ...
  describe "configuration" do
    ...
    test "passes through additional Oban job options" do
      assert {:ok, [job1, job2]} =
               PassthroughOptionsEventBus.emit(:passthrough_event, %{"test" => "data"})

      assert job1.meta == %{"source" => "handler"}
      assert job2.meta == %{"source" => "global"}
      # Defaults still apply alongside passthrough options
