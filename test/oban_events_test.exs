defmodule ObanEventsTest do
  ...
  defmodule PerHandlerOptionsEventBus do
    ...
    @events %{
      multi_handler_event: [
        {TestHandler, oban: [priority: 0, max_attempts: 5, tags: ["critical"], schedule_in: 60]},
  ...
  describe "configuration" do
    ...
    test "per-handler options override global defaults" do
      ...
      # First handler: custom priority, max_attempts, tags, and schedule_in
      assert job1.priority == 0
      assert job1.max_attempts == 5
      assert job1.tags == ["critical"]
      assert DateTime.diff(job1.scheduled_at, DateTime.utc_now(), :second) > 30
      # Uses global default
