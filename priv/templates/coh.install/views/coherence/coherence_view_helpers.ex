defmodule <%= web_base %>.Coherence.ViewHelpers do
  ...
  * :register - customize the register link text (#{@register_link})
  ...
  # These must be evaluated at runtime so the text follows the current locale.
  # Module attributes would be translated once, at compile time.
  defp recover_link_text,  do: dgettext("coherence", "Forgot your password?")
  defp unlock_link_text,   do: dgettext("coherence", "Send an unlock email")
  defp register_link_text, do: dgettext("coherence", "Need An Account?")
  defp invite_link_text,   do: dgettext("coherence", "Invite Someone")
  defp confirm_link_text,  do: dgettext("coherence", "Resend confirmation email")
  defp signin_link_text,   do: dgettext("coherence", "Sign In")
  defp signout_link_text,  do: dgettext("coherence", "Sign Out")
