...
By default `:required` is set to `true`, so it will handle error when resource is not loaded. Set `required: false` to allow a missing resource.
...
| `:non_id_actions` | **Deprecated**, use a separate `:authorize_resource` with `required: false` instead. See the [upgrade guide](upgrade.md) **Available only in Canary.Plugs** | `[:find_by_name]` |
...
`Canary.Plugs` and `Canary.Hooks` share the same API and options, so you can use the same opts with `plug` and `mount_canary`. The only exception is loading all resources for non-id actions, which is supported only by the (deprecated) plug options.
...
The `assigns.public_event` might be set to `nil` if there is no matchin `Event`. By default the `:required` option is `true`, so the `not_found_handler` will be called. Set `required: false` to allow it.
...
By default, when the action is `:index`, all records from the specified model will be loaded. This can be overridden to fetch a single record from the database by setting `required: true` explicitly.
