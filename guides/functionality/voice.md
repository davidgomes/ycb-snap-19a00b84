...
is required for all voice sessions. Nostrum negotiates it automatically, and like the
transport encryption modes below, it is invisible to the user.

Nostrum uses the [`:dave` package](https://hex.pm/packages/dave), which is backed by
Rust NIF bindings to [`davey`](https://github.com/Snazzah/davey).
...
If you have an exceptionally uncommon OS or you prefer to build from source with your
installed Rust toolchain, you can add `Rustler` to your project dependencies and set
`FORCE_DAVE_BUILD=true` when compiling.
...
DAVE encryption is end-to-end, and the transport encryption discussed in the next section
sits on top of the DAVE encrypted opus frames.

Incoming audio from a user can only be decrypted once their SSRC is known, which
happens when they start speaking. Packets that can't be decrypted are discarded.
