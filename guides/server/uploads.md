...
`max_entries_mode: :total` to also count consumed entries for the lifetime of an
upload configuration, for example when auto uploads are consumed as they complete.
Once all entries have been consumed or cancelled, call [`allow_upload/3`] again to
start a new batch and reset a `:total` limit.
