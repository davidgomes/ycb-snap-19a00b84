...
A cursor comparison follows the same `NULL` placement as the order direction.
`NULL > 7` is never true, so when `NULL` sorts last the predicate also keeps
rows where the field is `NULL`, and when the cursor value itself is `NULL` the
next page is the rows that sort after `NULL`.

```elixir
%{first: 2, order_by: [:age, :id]}
```

With the default ascending order, PostgreSQL sorts `NULL` last:
...
`:asc` and `:desc` use the adapter default: PostgreSQL sorts `NULL` last when
ascending and first when descending, and SQLite and MySQL do the opposite.
`:asc_nulls_first`, `:asc_nulls_last`, `:desc_nulls_first` and
`:desc_nulls_last` name the position on every adapter. A `NULL` cursor value
only separates rows when a later order field does, so keep a unique field such
as the primary key in the order.
