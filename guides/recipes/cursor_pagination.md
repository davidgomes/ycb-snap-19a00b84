...
Nullable columns are supported. The cursor comparison follows the null
placement of the order direction, so rows whose order value is `NULL` stay in
the result.
...
The plain order directions `:asc` and `:desc` leave the placement of null
values to the database. PostgreSQL sorts nulls last ascending and first
descending, while SQLite and MySQL do the opposite. The four explicit
directions `:asc_nulls_first`, `:asc_nulls_last`, `:desc_nulls_first` and
...
`:desc` raises when no repo can be resolved. A unique field still has to be
part of the order, or rows that share an order value — including several
`NULL`s — are skipped.
