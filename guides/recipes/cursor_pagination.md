...
Nullable columns are supported. The cursor comparison follows the same null
placement as the order.
...
The table shows PostgreSQL. The plain order directions `:asc` and `:desc` leave
the placement of null values to the database. PostgreSQL sorts nulls last
ascending and first descending, while SQLite and MySQL do the opposite. The
four explicit directions `:asc_nulls_first`, `:asc_nulls_last`,
...
`:desc` raises when no repo can be resolved. A second order field still has to
make the order unique: several rows can share a `NULL`.
