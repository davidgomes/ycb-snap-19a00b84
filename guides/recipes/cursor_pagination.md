...
Ordering by a nullable column builds a predicate that accounts for where the
database sorts the `NULL` values.
...
PostgreSQL sorts `NULL` last for a plain `:asc` direction, so the cursor
predicate for the `age` field also matches rows where it `IS NULL` once the
cursor has passed the last non-`NULL` value. A second order field, `id` here,
still breaks ties between rows that share a `NULL`.

Flop determines where the database sorts `NULL` values for the plain `:asc`
and `:desc` directions from the repo's Ecto adapter, which is why cursor
pagination with those directions requires the repo to be configured, either
by passing it to the query functions or by setting it up in the application
environment or on a backend module. Use the `:asc_nulls_first`,
`:asc_nulls_last`, `:desc_nulls_first` or `:desc_nulls_last` directions
instead if you want to avoid that requirement, since they already say where
`NULL` goes.
