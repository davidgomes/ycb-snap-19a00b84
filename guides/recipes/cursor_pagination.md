...
Nullable columns are supported. With `order_by: [:age, :id]`, the rows whose
age is `NULL` stay in the result.
...
The plain directions `:asc` and `:desc` leave the placement of null values to
the database. PostgreSQL sorts nulls last ascending and first descending, while
SQLite and MySQL do the opposite. `:asc_nulls_first`, `:asc_nulls_last`,
`:desc_nulls_first` and `:desc_nulls_last` mean the same on every database.

The cursor comparison has to match that sort. Flop reads the placement from the
repo's Ecto adapter, so cursor pagination with `:asc` or `:desc` raises when no
repo can be resolved. Pass `repo:` or use a direction that states where nulls
go.
