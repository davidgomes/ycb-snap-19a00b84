...
a list instead of a row, so the value is `nil`. Flop reads a `nil` cursor value
as `NULL`, so the next page starts at the position of a `NULL` toy name rather
than at the last row. Ordering by `[:toy_name, :name]` with `first: 2`, the
first page holds two of Alpha's four rows. Where `NULL`s sort last, the next
page is empty; where they sort first, it starts over with the first toy.
