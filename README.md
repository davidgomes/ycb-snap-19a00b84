...
Any other option accepted by [`Oban.Job.new/2`](https://hexdocs.pm/oban/Oban.Job.html#new/2) can be set in the global `:oban` tuple (`meta`, `unique`, `schedule_in`, and so on). Those values become the default for every handler.
...
Any option accepted by [`Oban.Job.new/2`](https://hexdocs.pm/oban/Oban.Job.html#new/2) can be set under the `:oban` key. Per-handler values are merged over the global defaults, so you only need to specify the options you want to override. Each key is replaced as a whole (`tags`, `meta`, and `unique` are not deep-merged).
...
- `meta` - Extra job metadata (map)
- `unique` - Uniqueness constraints (keyword list)
- `schedule_in` - Delay execution (integer seconds or `{n, unit}`)
- `scheduled_at` - Run at a specific `DateTime`
