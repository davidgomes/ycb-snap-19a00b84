...
Any `Oban.Job` option is accepted here (`:queue`, `:max_attempts`, `:priority`, `:tags`,
`:unique`, `:meta`, ...).
...
Any `Oban.Job` option can be given under the `:oban` key, for example:
...
- `unique`, `meta`, `schedule_in`, ... - Any other option accepted by `Oban.Job.new/2`

Per-handler options are merged over the global options, so a handler only needs to
