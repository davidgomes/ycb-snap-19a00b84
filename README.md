...
This library also includes `mix` tasks for Phoenix 1.3 style projects:

`mix phx.gen.html.slime`
`mix phx.gen.layout.slime`

The first task creates a html resource in the same way `phx.gen.html` does
with the exception that all templates are `.slime` files instead of `.eex` files.

The second task creates a new `lib/my_app_web/templates/layout/app.html.slime`
with the exact same content as the `app.html.eex` file. Do not forget to remove
the old `app.html.eex` file.

For projects using the pre Phoenix 1.3 `web/` directory structure, the
`mix phoenix.gen.html.slime` and `mix phoenix.gen.layout.slime` tasks are still
available and generate files in `web/` instead.
