...
With MySQL, the producer is not notified when jobs are inserted: new jobs wait for
the next poll (see `poll_interval`) when the workers are idle, and the `:notify`
