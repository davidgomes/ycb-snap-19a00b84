...
Since MySQL has no notification mechanism, new jobs are dispatched when the queue
is polled or when workers request more jobs, so consider lowering the `poll_interval`.
