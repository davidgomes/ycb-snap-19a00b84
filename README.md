...
Accepts any `Oban.Worker.new/2` option. Defaults for options not provided:
- `queue`: `:oban_events`
- `max_attempts`: `3`
...
Oban job options (under the `:oban` key) are merged over the global defaults and accept
any `Oban.Worker.new/2` option, e.g. `queue`, `max_attempts`, `priority`, `tags`, `schedule_in`,
`unique`, `meta`.
