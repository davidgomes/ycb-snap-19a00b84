...
Any [`Oban.Job.new/2`](https://hexdocs.pm/oban/Oban.Job.html#new/2) option can be passed under the `:oban` key and is merged over the global options. Common options:
...
The same options can also be set globally in the `use ObanEvents, oban: {MyApp.Oban, ...}` config.

`:oban` is the only supported handler option; anything else raises a compile error.
