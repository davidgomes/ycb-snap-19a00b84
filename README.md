...
For projects using the pre-1.3 `web/` directory structure, the `phoenix`
prefixed tasks are still available:

`mix phoenix.gen.html.slime`
`mix phoenix.gen.layout.slime`

They behave the same way, but generate files in `web/` instead of `lib/my_app_web/`.
