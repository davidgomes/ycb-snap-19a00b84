...
Implement the `ObanEvents.Handler` behaviour. Handlers receive an `ObanEvents.Event` struct containing your data plus [metadata](#event-metadata):
...
`use ObanEvents.Handler` aliases `ObanEvents.Event`, so you can match on `%Event{}` directly.
...
Handlers receive an `ObanEvents.Event` struct with your data plus metadata for deduplication, tracing, and correlation:

```elixir
%ObanEvents.Event{
  data: %{"user_id" => 123},                               # Your event data (string keys)
  event_id: "01933b7e-8a3f-7f6f-9e42-6c8f3a0b2d1e",        # Unique per emit
  idempotency_key: "01933b7e-9f2c-7a1b-8d4e-3c5f6a7b8c9d", # Unique per handler job
  causation_id: "01933b7e-7f4a-7c2d-9b3e-4d5f6a7b8c9d",    # Optional: event_id of the parent event
  correlation_id: "01933b7e-6e3b-7d5f-8c4e-5d6f7a8b9c0d"   # Optional: groups a business operation
}
```

`event_id` and `idempotency_key` are generated automatically (UUIDv7). `causation_id` and `correlation_id` are optional and passed as options to `emit/3`.

### `idempotency_key` - Deduplicate side effects

Each handler job gets its own key, and it stays the same when the job is retried. Use it to make side effects safe to repeat, e.g. with an outbox table:

```elixir
def handle_event(:user_created, %Event{data: data, idempotency_key: key}) do
  %OutboxEmail{idempotency_key: key, user_id: data["user_id"], template: "welcome"}
...
### `event_id` and `causation_id` - Build event chains

All handlers of a single emit receive the same `event_id`. Pass it as `causation_id` when a handler emits a follow-up event:
...
### `correlation_id` - Group a business operation

Use the same `correlation_id` for every event of one operation, and propagate it when emitting from handlers:

```elixir
correlation_id = UUIDv7.generate()

Events.emit(:order_placed, %{order_id: order.id}, correlation_id: correlation_id)
Events.emit(:payment_captured, %{order_id: order.id}, correlation_id: correlation_id)

def handle_event(:payment_captured, %Event{event_id: event_id, correlation_id: correlation_id, data: data}) do
  Events.emit(:invoice_requested, data, causation_id: event_id, correlation_id: correlation_id)
end
```
...
Emit an event to all registered handlers. `opts` is optional, so `emit/2` works too.
...
MyApp.Events.emit(:welcome_email_requested, %{user_id: 123},
...
- `:causation_id` - The `event_id` of the event that caused this emit
...
When there's no natural unique key, use the event's `idempotency_key` (see [Event Metadata](#idempotency_key---deduplicate-side-effects)).
...
- Handlers now receive an `%ObanEvents.Event{}` instead of the data map. Change `handle_event(name, data)` clauses to `handle_event(name, %Event{data: data})`.
- Jobs enqueued by 0.1 don't carry `event_id` or `idempotency_key`, and the 0.2 `DispatchWorker` rejects them as invalid. Let your events queue(s) drain before deploying the upgrade.
...
Emitted jobs also carry `"event_id"`, `"idempotency_key"`, `"causation_id"`, and `"correlation_id"` args. `assert_enqueued` matches args partially, so you only need to include the ones you care about.

### Testing Handlers

Use `ObanEvents.Testing.build_event/2` to build an `ObanEvents.Event` and call the handler directly:
...
`build_event/2` generates `event_id` and `idempotency_key`, and leaves `causation_id` and `correlation_id` as `nil`. The data is round-tripped through JSON, so the handler receives string keys just like in production. Any metadata field can be overridden:
