...
Oban job options (under `:oban` key) are merged over the global options, so any
`Oban.Job.new/2` option can be set, for example:
...
- `unique`, `schedule_in`, etc. - Any other Oban job option
