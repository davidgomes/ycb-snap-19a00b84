...
This library also includes `mix` tasks for Phoenix 1.3:
...
The `gen.html.slime` tasks create a html resource in the same way `phx.gen.html`
and `phoenix.gen.html` do with the exception that all templates are `.slime`
files instead of `.eex` files.

The `gen.layout.slime` tasks create a new `app.html.slime` layout
(in `lib/my_app_web/templates/layout` for `phx` and `web/templates/layout` for
