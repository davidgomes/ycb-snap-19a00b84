...
The `secret_key_base` can usually be found in the Rails' `secrets.yml` file. Since Rails 5.2, `secret_key_base` in test and development is derived as a MD5 hash of the application's name. To fetch the key value you can run:
...
The `secret_key_base` should be copied to Phoenix's `config.exs` file. There should already be a key named like that and you should override it. It must be at least 32 bytes long.
