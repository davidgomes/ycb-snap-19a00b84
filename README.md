...
Put Oban job options under the `:oban` key. They are merged over the module defaults, and every option accepted by `Oban.Job.new/2` is forwarded:
...
- `unique` - Uniqueness period and fields
- `schedule_in` / `scheduled_at` - Delay execution
- `meta` - Extra job metadata

```elixir
{MyApp.EmailHandler,
 oban: [priority: 0, schedule_in: 30, unique: [period: 60, fields: [:worker, :args]]]}
```

The same keys work on the module-level `:oban` tuple. A key set on a handler replaces that key from the module defaults; keys you omit are kept.
