...
Any option accepted by `Oban.Worker.new/2` can be set under the `:oban` key. Common ones:
...
- `unique`, `schedule_in`, `meta`, and the rest of Oban's job options

Per-handler values replace the matching global option. Unspecified keys keep the global value.
