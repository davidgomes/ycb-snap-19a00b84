...
Anything `Oban.Job.new/2` accepts, nested under the `:oban` key. Handler options are merged on top of the module defaults, so unspecified keys keep the global value.
...
- `unique` - Oban uniqueness period and fields
- `schedule_in` / `scheduled_at` - Delay the job
- `meta` - Extra job metadata (replaces the module default when both set a `:meta` key)
