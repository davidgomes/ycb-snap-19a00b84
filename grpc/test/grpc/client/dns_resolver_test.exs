defmodule GRPC.Client.ReResolveTest do
  ...
  setup do
    ...
    :telemetry.attach_many(
      ...
      fn name, _measurements, _metadata, _config ->
        send(test_pid, {:resolve_event, self(), name})
      end,
      nil
    )
  ...
  defp await_resolution(ref, kind) do
    resolver_pid = get_state(ref).resolver_state.worker_pid
    do_await_resolution(resolver_pid, [:grpc, :client, :resolve, kind])

    # Telemetry is emitted before the update is sent to the Connection.
    :sys.get_state(resolver_pid)
    get_state(ref)
  end

  defp do_await_resolution(resolver_pid, event) do
    receive do
      {:resolve_event, ^resolver_pid, ^event} -> :ok
      {:resolve_event, ^resolver_pid, _other} -> do_await_resolution(resolver_pid, event)
    after
      1_000 -> flunk("timed out waiting for #{inspect(event)}")
    end
  end
  ...
  describe "scale-up: new backends discovered" do
    test "adds channels for addresses that appear in DNS", ctx do
      ...
      wait_until(fn -> map_size(get_state(ctx.ref).real_channels) == 2 end)
  ...
  describe "scale-down: backends removed" do
    test "disconnects channels for addresses no longer in DNS", ctx do
      ...
      wait_until(fn -> map_size(get_state(ctx.ref).real_channels) == 1 end)
  ...
  describe "no-op: unchanged addresses" do
    test "leaves channels untouched when DNS returns the same set", ctx do
      ...
      state_after = await_resolution(ctx.ref, :stop)
  ...
  describe "complete replacement: disjoint address lists" do
    test "removes old and creates new channels for entirely different backends", ctx do
      ...
      wait_until(fn -> Map.has_key?(get_state(ctx.ref).real_channels, "10.0.0.3:50051") end)
  ...
  describe "DNS failure during re-resolution" do
    test "keeps existing channels on resolver error", ctx do
      ...
      state = await_resolution(ctx.ref, :error)
      assert map_size(state.real_channels) == 1
      assert Map.has_key?(state.real_channels, "10.0.0.1:50051")
  ...
  describe "recovery after DNS failure" do
    test "next successful cycle updates channels after a failed one", ctx do
      ...
      assert map_size(await_resolution(ctx.ref, :error).real_channels) == 1
  ...
  describe "pick_first reconciliation" do
    test "pick_channel returns the new backend after DNS replaces it", ctx do
      ...
      wait_until(fn -> Map.has_key?(get_state(ctx.ref).real_channels, "10.0.0.9:50051") end)
  ...
  describe "pick_channel after full backend replacement" do
    test "picks a channel from the new backend set", ctx do
      ...
      wait_until(fn -> Map.has_key?(get_state(ctx.ref).real_channels, "10.0.0.99:50051") end)
  ...
  describe "repeated re-resolution cycles" do
    test "timer fires on every interval tick, accumulating changes", ctx do
      ...
      wait_until(fn -> map_size(get_state(ctx.ref).real_channels) == 3 end)
  ...
  describe "re-resolution after disconnect" do
    test "linked resolver dies when connection disconnects", ctx do
      ...
      assert_receive {:DOWN, ^resolver_mon, :process, ^resolver_pid, _reason}, 1_000
  ...
  describe "port change on same host" do
    test "detects port change as a new address", ctx do
      ...
      wait_until(fn -> Map.has_key?(get_state(ctx.ref).real_channels, "10.0.0.1:50052") end)
  ...
  describe "exponential backoff on failure" do
    test "interval doubles after each consecutive failure", ctx do
      ...
      await_resolution(ctx.ref, :error)
  ...
  describe "telemetry events" do
    ...
    test "emits :stop event on successful re-resolution", ctx do
      ...
      assert_receive {:telemetry, [:grpc, :client, :resolve, :stop], measurements,
                      %{address_count: 2} = metadata},
                     1_000
    ...
    test "emits :error event on DNS failure", ctx do
      ...
      assert_receive {:telemetry, [:grpc, :client, :resolve, :error], measurements, metadata},
                     1_000
    ...
    test "emits :error event on empty address list", ctx do
      ...
      assert_receive {:telemetry, [:grpc, :client, :resolve, :error], _measurements, metadata},
                     1_000
  ...
  describe "unhealthy-pick fallback" do
    ...
    test "pick_channel returns error when all new channels fail", ctx do
      ...
      wait_until(fn -> not Map.has_key?(get_state(ctx.ref).real_channels, "10.0.0.1:50051") end)
  ...
  describe "retry previously failed channels" do
    ...
    test "reconnects a previously failed channel when it becomes reachable", ctx do
      ...
      wait_until(fn ->
        match?({:connected, _}, Map.get(get_state(ctx.ref).real_channels, "10.0.0.2:50051"))
      end)
  ...
  describe "dedicated resolver process" do
    test "Connection stays responsive during slow DNS resolution", ctx do
      ...
      assert_receive :resolving, 1_000
  ...
  describe "refresh handler with failed channels" do
    ...
    test "GenServer survives when :refresh picks a failed channel", ctx do
      ...
      # Sync with the Connection so the :refresh messages are processed
      _ = :sys.get_state(pid)
  ...
  describe "resolver worker crash recovery" do
    test "Connection re-initializes resolver when worker crashes", ctx do
      ...
      wait_until(fn ->
        ...
        worker_pid != original_pid and Process.alive?(worker_pid)
      end)
