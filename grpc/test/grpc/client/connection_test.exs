defmodule GRPC.Client.ConnectionTest do
  ...
  describe "pick_channel/2" do
    ...
    test "returns {:ok, channel} when a channel is stored in the ETS table", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
  ...
  describe "disconnect/1" do
    ...
    test "pick_channel returns {:error, :no_connection} after disconnect (ETS entry is erased)",
  ...
  describe "terminate/2 - ETS cleanup on process kill" do
    test "ETS entry is erased when process is killed without disconnect", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
