defmodule GRPC.Client.ConnectionTest do
  ...
  describe "pick_channel/2" do
    test "returns {:error, :no_connection} when no connection is registered", %{ref: ref} do
      channel = %Channel{ref: ref}

      assert {:error, :no_connection} = Connection.pick_channel(channel)
    end

    test "returns {:ok, channel} for a registered connection", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
  ...
  describe "cleanup on process kill" do
    test "pick_channel fails when process is killed without disconnect", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
