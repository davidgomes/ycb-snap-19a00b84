defmodule GRPC.Client.ConnectionTest do
  ...
  describe "pick_channel/2" do
    test "returns {:error, :no_connection} when no ETS entry exists", %{ref: ref} do
      channel = %Channel{ref: ref}

      assert {:error, :no_connection} = Connection.pick_channel(channel)
    end

    test "returns {:ok, channel} when a channel is stored in the LB ETS table", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
  ...
  describe "disconnect/1" do
    ...
    test "pick_channel returns {:error, :no_connection} after disconnect (ETS entry is deleted)",
  ...
  describe "terminate/2 - ETS cleanup on process kill" do
    test "the LB ETS entry is deleted when process is killed without disconnect", %{
      ref: ref,
      target: target,
      adapter: adapter
    } do
