defmodule GRPC.Client.Adapters.MintTest do
  ...
  describe "connect/2" do
    ...
    test "defaults client settings when none is passed", %{port: port} do
      ...
      wait_until(fn ->
        match?(
          %{initial_window_size: 8_000_000, max_frame_size: 8_000_000},
          Map.get(:sys.get_state(conn_pid).conn, :client_settings)
        )
      end)
    ...
    test "allow client settings to be passed", %{port: port} do
      ...
      wait_until(fn ->
        match?(
          %{initial_window_size: 50_000, max_frame_size: 50_000},
          Map.get(:sys.get_state(conn_pid).conn, :client_settings)
        )
      end)
