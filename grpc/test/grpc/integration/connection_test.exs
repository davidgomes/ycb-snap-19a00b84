defmodule GRPC.Integration.ConnectionTest do
  ...
  test "reconnection works" do
    ...
    # The Gun adapter's default retry_fun waits ~1s before its first reconnect
    retry_fun = fn retries, _opts -> %{retries: retries - 1, timeout: 10} end
    {:ok, channel} = GRPC.Stub.connect("localhost:#{port}", adapter_opts: [retry_fun: retry_fun])
