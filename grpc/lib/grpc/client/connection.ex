defmodule GRPC.Client.Connection do
  ...
  * Every RPC picks its channel through the load-balancing module, in the
    caller's process, without a round trip to the orchestrator.
  * The orchestrator keeps the load balancer's channel set in sync with the
    resolver.
  ...
  ## Notes
    ...
    * The load balancer state is published to `:persistent_term` once per
      connection. Re-resolution updates the load balancer in place, so it does
      not trigger the node-wide garbage collection pass of a
      `:persistent_term` write.
  """
  use GenServer
  alias GRPC.Channel
  ...
  def child_spec({%__MODULE__{} = initial_state, _lb_channels} = init_arg) do
    %{
      id: {__MODULE__, initial_state.virtual_channel.ref},
      start:
        {GenServer, :start_link,
         [__MODULE__, init_arg, [name: via(initial_state.virtual_channel.ref)]]},
      restart: :transient,
      type: :worker,
      shutdown: 5000
    }
  ...
  @impl GenServer
  def init({%__MODULE__{} = state, lb_channels}) do
    Process.flag(:trap_exit, true)

    # The load balancer is initialized here rather than in connect/2 so that
    # this process, not the caller, owns its ETS tables.
    {:ok, lb_state} = state.lb_mod.init(channels: lb_channels)
    ...
    # Published last: a crash in init/1 skips terminate/2 and would leak the entry.
    :persistent_term.put(lb_key(state.virtual_channel.ref), {state.lb_mod, lb_state})
  ...
  def connect(target, opts \\ []) do
    case build_initial_state(target, opts) do
      {:ok, initial_state, lb_channels} ->
        ch = initial_state.virtual_channel

        case DynamicSupervisor.start_child(
               GRPC.Client.Supervisor,
               child_spec({initial_state, lb_channels})
             ) do
          {:ok, _pid} ->
            {:ok, ch}
  ...
  automatically on every RPC – but it can be useful when debugging or testing.

  The pick runs in the calling process and does not message the orchestrator,
  so consecutive calls may return different channels (e.g. with `:round_robin`).

  Returns:

    * `{:ok, channel}` – the chosen `GRPC.Channel`
    * `{:error, :no_connection}` – if the orchestrator is not available or has
      no connected channel to offer
    ...
    case :persistent_term.get(lb_key(ref), nil) do
      {lb_mod, lb_state} ->
        case lb_mod.pick(lb_state) do
          ...
          {:error, _reason} -> {:error, :no_connection}
        end
  ...
  def handle_call({:disconnect, %Channel{adapter: adapter} = channel}, _from, state) do
    # Erased before the transports close so in-flight picks fail fast with
    # :no_connection instead of handing out a channel that is being torn down.
  ...
  defp rebalance_after_reconcile(new_addresses, real_channels, state) do
    channels = lb_channels(new_addresses, real_channels)

    if channels == [] do
      Logger.warning("No healthy channels available after re-resolution")
    end

    case state.lb_mod.update(state.lb_state, channels) do
      {:ok, lb_state} ->
        if lb_state != state.lb_state do
          :persistent_term.put(lb_key(state.virtual_channel.ref), {state.lb_mod, lb_state})
        end
  ...
  # Connected channels in resolver order, which is the order the LB sees them in.
  defp lb_channels(addresses, real_channels) do
    Enum.flat_map(addresses, fn %{address: host, port: port} ->
      case Map.get(real_channels, build_address_key(host, port)) do
        {:connected, %Channel{} = channel} -> [channel]
        _missing_or_failed -> []
      end
    end)
  end

  defp shutdown_lb(%{lb_mod: lb_mod, lb_state: lb_state} = state)
  ...
  defp build_initial_state(target, opts) do
    ...
    case resolver.resolve(norm_target) do
      ...
      {:error, _reason} ->
        build_direct_state(base_state, norm_target, lb_policy_opt, norm_opts, adapter)
    end
  end
  ...
  defp build_balanced_state(
    ...
    case lb_channels(addresses, real_channels) do
      [first | _] = channels ->
        {:ok,
         %__MODULE__{
           base_state
           | lb_mod: choose_lb(lb_policy),
             virtual_channel: first,
             real_channels: real_channels
         }, channels}

      [] ->
        {:error, connect_failure_reason(addresses, real_channels)}
    end
  end

  defp connect_failure_reason([], _real_channels), do: :no_addresses

  defp connect_failure_reason([%{address: host, port: port} | _], real_channels) do
    {:failed, reason} = Map.fetch!(real_channels, build_address_key(host, port))
    reason
  end

  defp build_direct_state(
         %__MODULE__{} = base_state,
         norm_target,
         lb_policy_opt,
         norm_opts,
    ...
    case connect_real_channel(vc, host, port, norm_opts, adapter) do
      {:ok, ch} ->
        {:ok,
         %__MODULE__{
           base_state
           | lb_mod: choose_lb(lb_policy_opt),
             virtual_channel: ch,
