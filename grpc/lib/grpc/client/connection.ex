defmodule GRPC.Client.Connection do
  ...
  * Each call to `pick_channel/2` dispatches to the LB module, which selects a channel
  ...
  def init(%__MODULE__{} = state) do
    ...
    case state.lb_mod.init(channels: connected) do
      {:ok, lb_state} ->
        ...
        # The LB module owns its own ETS table (created above), keyed off this
        # process; only the {lb_mod, lb_state} handle is published so pick_channel/2
        # can dispatch to it per request without going through the GenServer.
