defmodule GRPC.Client.Connection do
  ...
  * The current set of real channels and LB state live in an ETS table
    (see `GRPC.Client.Connection.LBStore`), so `pick_channel/2` performs a
    fresh LB pick on every call directly from the calling process, without
    going through the orchestrator process.
  ...
  ## Notes
    ...
    * Each call to `pick_channel/2` performs its own load-balancing pick
      (e.g. round-robin advances to the next backend per request).
  """
  use GenServer
  alias GRPC.Channel
  alias GRPC.Client.Connection.LBStore
  ...
  def init(%__MODULE__{} = state) do
    ...
    # only now publish the chosen channel and LB state to ETS (which should
    # already have adapter_payload because build_initial_state connected real
    # channels and set virtual_channel)
    sync_lb_table(state)
  ...
  Each call performs its own load-balancing pick against the ETS-backed
  state for the connection (e.g. round-robin advances to the next backend
  on every call), so this reads and updates state directly from the calling
  process rather than going through the orchestrator `GenServer`.
  ...
  def pick_channel(%Channel{ref: ref} = _channel, _opts \\ []) do
    case :ets.lookup(LBStore.table(), ref) do
      [{^ref, nil, _lb_state, _real_channels, %Channel{} = virtual_channel}] ->
        {:ok, virtual_channel}

      [{^ref, lb_mod, lb_state, real_channels, %Channel{} = virtual_channel}] ->
        {:ok, per_request_pick(ref, lb_mod, lb_state, real_channels, virtual_channel)}
  ...
  defp sync_lb_table(%__MODULE__{virtual_channel: %Channel{ref: ref}} = state) do
    :ets.insert(
      LBStore.table(),
      {ref, state.lb_mod, state.lb_state, state.real_channels, state.virtual_channel}
    )
  end

  defp erase_lb_table(ref) do
    :ets.delete(LBStore.table(), ref)
  end

  defp per_request_pick(ref, lb_mod, lb_state, real_channels, virtual_channel) do
    case lb_mod.pick(lb_state) do
      {:ok, {host, port}, new_lb_state} ->
        :ets.update_element(LBStore.table(), ref, {3, new_lb_state})

        case Map.get(real_channels, build_address_key(host, port)) do
          {:connected, %Channel{} = picked_channel} -> picked_channel
          _ -> virtual_channel
        end
