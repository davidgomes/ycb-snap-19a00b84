defmodule GRPC.Client.Connection do
  ...
  * The load-balancing state is published in the node-local `Registry` entry of
    the orchestrator, so every call to `pick_channel/2` runs the LB policy in the
    caller process without messaging the orchestrator.
  ...
  @type t :: %__MODULE__{
          ...
          addresses: [%{address: term(), port: non_neg_integer()}],
          lb_mod: module() | nil,
          lb_state: term() | nil,
          resolver: module() | nil,
          adapter: module(),
  ...
  def init(%__MODULE__{} = state) do
    ...
    # The LB is initialized here rather than in build_initial_state/2 so that any
    # ETS tables it creates are owned by this process and not by the caller.
    {:ok, lb_state} =
      state.lb_mod.init(channels: connected_channels(state.addresses, state.real_channels))

    publish_lb(state.lb_mod, lb_state, state.virtual_channel.ref)
  ...
  def pick_channel(%Channel{ref: ref} = _channel, _opts \\ []) do
    with [{_pid, {lb_mod, lb_state}}] <- Registry.lookup(GRPC.Client.Registry, {__MODULE__, ref}),
    ...
    # The LB's ETS tables are gone once the orchestrator exits, which can race
    # with the asynchronous cleanup of its Registry entry.
  ...
  defp rebalance_after_reconcile(new_addresses, real_channels, state) do
    ...
    case state.lb_mod.update(state.lb_state, channels) do
      ...
      {:error, reason} ->
        Logger.warning("Failed to update load balancer: #{inspect(reason)}")
        state
    end
  end
  ...
  defp connected_channels(addresses, real_channels) do
    Enum.flat_map(addresses, fn %{address: host, port: port} ->
      case Map.get(real_channels, build_address_key(host, port)) do
        {:connected, %Channel{} = ch} -> [ch]
        _ -> []
      end
    end)
  end

  defp publish_lb(lb_mod, lb_state, ref) do
    Registry.update_value(GRPC.Client.Registry, {__MODULE__, ref}, fn _ -> {lb_mod, lb_state} end)
  end

  defp unpublish_lb(ref) do
    Registry.update_value(GRPC.Client.Registry, {__MODULE__, ref}, fn _ -> nil end)
  end
  ...
  defp build_balanced_state(
    ...
    case connected_channels(addresses, real_channels) do
      ...
      [] ->
        {:error, first_failure(addresses, real_channels)}
    end
  end

  defp first_failure([], _real_channels), do: :no_addresses

  defp first_failure([%{address: host, port: port} | _], real_channels) do
    case Map.get(real_channels, build_address_key(host, port)) do
