defmodule GRPC.Client.Connection do
  ...
  * The load-balancer state lives in an ETS table owned by the orchestrator, so
    every request picks a channel in the caller process without a `GenServer` call.
  ...
  def init(%__MODULE__{} = state) do
    ...
    # The LB is initialized here, not in connect/2, so that the ETS tables it
    # creates are owned by this process and die with it.
  ...
  def pick_channel(%Channel{ref: ref} = _channel, _opts \\ []) do
    ...
    # The LB table is deleted when the connection shuts down, which can race
    # with a lookup that already returned its reference.
  ...
  defp publish_lb(ref, value) do
    Registry.update_value(GRPC.Client.Registry, {__MODULE__, ref}, fn _ -> value end)
  end
  ...
  defp rebalance_after_reconcile(new_addresses, real_channels, state) do
    ...
    {:ok, lb_state} = state.lb_mod.update(state.lb_state, channels)

    %{state | addresses: new_addresses, real_channels: real_channels, lb_state: lb_state}
  end
