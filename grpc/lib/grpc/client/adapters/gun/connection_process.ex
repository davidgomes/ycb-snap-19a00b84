if Code.ensure_loaded?(:gun) do
  defmodule GRPC.Client.Adapters.Gun.ConnectionProcess do
    ...
    bookkeeping. Named connection processes are registered in the node-local
        ...
        {:ok, gun_pid} ->
          case :gun.await_up(gun_pid) do
            {:ok, :http2} ->
              ...
              {:ok,
               %{gun_pid: gun_pid, gun_monitor_ref: gun_monitor_ref, response_processes: %{}}}
    ...
    @impl GenServer
    def handle_call(:disconnect, _from, %{gun_pid: gun_pid, gun_monitor_ref: monitor_ref} = state) do
    ...
    def handle_info(
          ...
          %{gun_monitor_ref: monitor_ref} = state
        ) do
      Enum.each(state.response_processes, fn {_stream_ref, {response_pid, _ref}} ->
        send(response_pid, {:connection_down, reason})
      end)
