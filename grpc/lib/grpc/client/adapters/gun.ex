if Code.ensure_loaded?(:gun) do
  defmodule GRPC.Client.Adapters.Gun do
    ...
    `conn_pid` is the pid of the process that owns the Gun connection. It lives
    as long as the channel, not the process that called `connect/2`.
    `stream_ref` and `response_pid` are stored per request in `GRPC.Client.Stream`.
    ...
    def send_request(
      ...
      put_stream_payload(stream, ConnectionProcess.request(conn_pid, path, headers, data))
    end
    ...
    def send_headers(
      ...
      headers = GRPC.Transport.HTTP2.client_headers_without_reserved(stream, opts)
      put_stream_payload(stream, ConnectionProcess.open_stream(conn_pid, path, headers))
    end

    defp put_stream_payload(stream, {:ok, %{stream_ref: stream_ref, response_pid: response_pid}}) do
      stream
      |> GRPC.Client.Stream.put_payload(:stream_ref, stream_ref)
      |> GRPC.Client.Stream.put_payload(:response_pid, response_pid)
    end

    # The connection process is gone; receiving on this stream reports the error.
    defp put_stream_payload(stream, {:error, _reason}) do
      stream
      |> GRPC.Client.Stream.put_payload(:stream_ref, nil)
      |> GRPC.Client.Stream.put_payload(:response_pid, nil)
    end
    ...
    def send_data(%{channel: channel, payload: %{stream_ref: stream_ref}} = stream, message, opts) do
      ...
      {:ok, data, _} = GRPC.Message.to_data(message, opts)
      ConnectionProcess.send_data(conn_pid, stream_ref, fin, data)
      stream
    end
    ...
    def end_stream(%{channel: channel, payload: %{stream_ref: stream_ref}} = stream) do
      conn_pid = channel.adapter_payload[:conn_pid]
      ConnectionProcess.send_data(conn_pid, stream_ref, :fin, "")
      stream
    end
    ...
    defp await(nil, _timeout) do
      {:error,
       GRPC.RPCError.exception(GRPC.Status.unavailable(), "the connection process is not alive")}
    end
    ...
    defp await(response_pid, timeout) do
      ...
      case StreamResponseProcess.await(response_pid, timeout) do
