if Code.ensure_loaded?(:gun) do
  defmodule GRPC.Client.Adapters.Gun do
    ...
    `conn_pid` is the pid of the process owning the Gun connection. The per-request
    `stream_ref` and `response_pid` are stored in `GRPC.Client.Stream`.
    ...
    def send_request(stream, message, opts) do
      response = do_send_request(stream, message, opts)
      put_response_payload(stream, response)
    end
    ...
    defp do_send_request(
      ...
      {:ok, response} = ConnectionProcess.request(conn_pid, path, headers, data)
      response
    end
    ...
    def send_headers(
      ...
      headers = GRPC.Transport.HTTP2.client_headers_without_reserved(stream, opts)
      {:ok, response} = ConnectionProcess.open_stream(conn_pid, path, headers)
      put_response_payload(stream, response)
    end

    defp put_response_payload(stream, %{stream_ref: stream_ref, response_pid: response_pid}) do
      stream
    ...
    def cancel(stream) do
      ...
      :ok = ConnectionProcess.cancel(conn_pid, stream_ref)
