...
* Add support for events `auto-recover`, `viewport-top`, `viewport-bottom`, `mounted`, `remove`, `connected`
    and `disconnected`, either as `:on-*` directives or as their corresponding `phx-*` attributes
