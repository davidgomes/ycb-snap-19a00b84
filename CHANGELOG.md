...
## v2.0.0-dev
  ...
  * Breaking changes
    * `:required` option defaults to `true`, use `required: false` for optional resources

  * Deprecations
    * `:persisted` and `:non_id_actions` options are deprecated and will be removed in Canary 2.1.0, see the upgrade guide
