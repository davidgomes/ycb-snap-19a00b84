...
- Support filtering by custom fields that only configure `field_dynamic`. The
  filter operator and value are applied to the returned expression. A `filter`
  function still takes precedence if both are set.
