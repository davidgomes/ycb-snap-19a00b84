...
- Support filtering by custom fields that configure `field_dynamic` and no
  `filter` callback. Flop applies the filter operator to the dynamic
  expression. A configured `filter` callback still takes precedence.
