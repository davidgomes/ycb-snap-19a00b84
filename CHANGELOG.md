...
- Support filtering by custom fields with `field_dynamic`. The filter operator
  is applied to the dynamic expression. A custom `filter` function still takes
  precedence when both are configured. A filterable custom field may set either
  callback.
