...
- [Jobs] Display awaitable signals in the job details page
  ...
  Add a section that decodes and displays signal payloads sent via `Oban.Pro.Worker.signal/2`.
  While a job is parked waiting, the section shows "Awaiting Signal" with the deadline. Once a
  signal arrives, it switches to "Received Signal".

- [Resolver] Add `format_signal/2` resolver callback

  This allows customizing the decoded signal output, mirroring what's available with
  `format_recorded/2`.
