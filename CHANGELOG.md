...
### Enhancements
* Dispatch a cancelable `phx:navigate-start` event on the clicked element before live navigation (`<.link navigate>` / `<.link patch>`). Calling `event.preventDefault()` cancels the navigation ([#4332](https://github.com/phoenixframework/phoenix_live_view/pull/4332))
