...
## v2.0.0-dev
  * Enhancements
    ...
    * added `:required` option, defaulting to true
    * plugs and hooks share resource naming, required checks, and deprecation warnings

  * Dependency changes
    * Elixir ~> 1.14 is now required

  * Deprecations
    * The `:non_id_actions` option is deprecated and will be removed in Canary 2.1.0. Use a separate `:authorize_resource` call with `required: false` for those actions, and `:except` to skip them on load.
