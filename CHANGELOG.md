...
## v2.0.0-dev
  * Enhancements
    ...
    * consistent API for plugs and hooks, `Canary.Plugs` and `Canary.Hooks` accept the same opts

  * Breaking changes
    * `:required` option defaults to `true`, set `required: false` when the resource is optional

  * Deprecations
    * `:persisted` and `:non_id_actions` options are deprecated, check the [upgrade guide](docs/upgrade.md)
