...
- **`data_table` filter and column menus live in the page, not the
  ...
  survives the patches that a filter or column toggle triggers. A table
  that only toggles columns still mounts that hook, so the Columns
  trigger is not a dead button.
